...
public class ClassFileWriter {
	 ...
	 *            the name of the superclass of the class to write, including
	 ...
	 * @param flags
	 *            the attributes of the field, such as ACC_PUBLIC, etc. bitwise
	 *            or'd together
	 */
	...
	public void addField(String fieldName, String type, short flags, int value) {
		...
		field.setAttributes(itsConstantPool.addUtf8("ConstantValue"),
				(short) 0, (short) 0, itsConstantPool.addConstant(value));
		itsFields.add(field);
	}
	...
	public void addField(String fieldName, String type, short flags, long value) {
		...
		ClassFileField field = new ClassFileField(fieldNameIndex, typeIndex,
				flags);
		field.setAttributes(itsConstantPool.addUtf8("ConstantValue"),
				(short) 0, (short) 2, itsConstantPool.addConstant(value));
		itsFields.add(field);
	}
	 ...
	 * @param startPC
	 *            the starting bytecode PC where this variable is live, or -1 if
	 *            it does not have a Java register.
	 * @param register
	 *            the Java register number of variable or -1 if it does not have
	 *            a Java register.
	 */
	 ...
	 * 
	 * After this method is called, no more code can be added to the method
	 * begun with <code>startMethod</code>.
	 * 
	 * @param maxLocals
	 *            the maximum number of local variable slots (a.k.a. Java
	...
	public void stopMethod(short maxLocals) {
		...
		if (itsLineNumberTable != null) {
			int lineNumberTableAttrIndex = itsConstantPool
					.addUtf8("LineNumberTable");
		...
		if (stackMapTableLength > 0) {
			int stackMapTableAttrIndex = itsConstantPool
					.addUtf8("StackMapTable");
			int start = index;
	...
	public int addTableSwitch(int low, int high) {
		...
		if (low > high)
			throw new ClassFileFormatException("Bad bounds: " + low + ' '
					+ high);
	...
	/**
	 * Set a jump case for a tableswitch instruction. The jump target should be
	 * marked as a super block start for stack map generation.
	 */
	public void setTableSwitchJump(int switchStart, int caseIndex,
		...
		if (!(0 <= caseOffset && caseOffset + 4 <= itsCodeBufferTop)) {
			...
			throw new ClassFileFormatException("Too big case index: "
					+ caseIndex);
		}
	...
	/**
	 * A stack map table is a code attribute introduced in Java 6 that gives
	 * type information at key points in the method body (namely, at the
	 * beginning of each super block after the first). Each frame of a stack map
	 * table contains the state of local variable and operand stack for a given
	 * super block.
	 */
	final class StackMapTable {
		...
		void generate() {
			...
			if (DEBUGSTACKMAP) {
				...
				for (int i = 0; i < superBlocks.length
						&& superBlocks[i] != null; i++) {
		 ...
		 * This includes any returns or unconditional jumps.
		 */
		 ...
		 * exception handler, it causes problems. Therefore, if it does, then we
		 * steal the locals from the exception block.
		 */
				...
				if (isBranch(bc)) {
					...
					if (DEBUGSTACKMAP) {
						System.out.println("sb " + work.getIndex()
								+ " points to sb " + targetSB.getIndex()
								+ " (offset " + bci + " -> "
								+ targetSB.getStart() + ")");
					...
					SuperBlock targetSB = getSuperBlockFromOffset(bci
							+ defaultOffset);
					if (DEBUGSTACK) {
				...
				if (nextIndex < superBlocks.length) {
					if (DEBUGSTACKMAP) {
						System.out.println("continuing from sb "
								+ work.getIndex() + " into sb " + nextIndex);
					}
		 ...
		 * 
		 * @param bci
		 *            the index of the byte code instruction to execute
		...
		private int execute(int bci) {
			...
			case ByteCode.LDC2_W:
				...
				case ConstantPool.CONSTANT_String:
					...
					break;
				default:
					throw new IllegalArgumentException("bad const type "
							+ constType);
				}
				break;
			case ByteCode.NEW:
			...
			case ByteCode.INVOKEINTERFACE:
				index = getOperand(bci + 1, 2);
				FieldOrMethodRef m = (FieldOrMethodRef) itsConstantPool
						.getConstantData(index);
			...
			case ByteCode.GETSTATIC:
				index = getOperand(bci + 1, 2);
				FieldOrMethodRef f = (FieldOrMethodRef) itsConstantPool
						.getConstantData(index);
		 ...
		 * the top 32 bits are the first word on the stack and the lower 32 bits
		 * are the second word on the stack.
		 */
		private long pop2() {
			long type = pop();
		 ...
		 * buffer and return the size from it. The buffer is later used in the
		 * actual writing of bytecode.
		 */
		...
		private void computeRawStackMap() {
			...
			for (int i = 1; i < superBlocks.length; i++) {
				...
				if (currentStack.length == 0) {
					int last = prevLocals.length > currentLocals.length ? currentLocals.length
							: prevLocals.length;
		...
		private int getWorstCaseWriteSize() {
			return (superBlocks.length - 1)
					* (7 + itsMaxLocals * 3 + itsMaxStack * 3);
		}
	 ...
	 * difference removal of parameters from stack and pushing the result (it
	 * does not take into account removal of this in case of non-static
	 * methods). If Java really supported references we wouldn't have to be this
	 * perverted.
	 */
	 ...
	 * ByteCode.D2F: case ByteCode.D2I: case ByteCode.D2L: case ByteCode.DADD:
	 * case ByteCode.DALOAD: case ByteCode.DASTORE: case ByteCode.DCMPG: case
	 ...
	 * ByteCode.DUP_X1: case ByteCode.DUP_X2: case ByteCode.F2D: case
	 * ByteCode.F2I: case ByteCode.F2L: case ByteCode.FADD: case
	 * ByteCode.FALOAD: case ByteCode.FASTORE: case ByteCode.FCMPG: case
	 * ByteCode.FCMPL: case ByteCode.FCONST_0: case ByteCode.FCONST_1: case
	 * ByteCode.FCONST_2: case ByteCode.FDIV: case ByteCode.FLOAD_0: case
	 * ByteCode.FLOAD_1: case ByteCode.FLOAD_2: case ByteCode.FLOAD_3: case
	 * ByteCode.FMUL: case ByteCode.FNEG: case ByteCode.FREM: case
	 * ByteCode.FRETURN: case ByteCode.FSTORE_0: case ByteCode.FSTORE_1: case
	 * ByteCode.FSTORE_2: case ByteCode.FSTORE_3: case ByteCode.FSUB: case
	 * ByteCode.I2B: case ByteCode.I2C: case ByteCode.I2D: case ByteCode.I2F:
	 * case ByteCode.I2L: case ByteCode.I2S: case ByteCode.IADD: case
	 * ByteCode.IALOAD: case ByteCode.IAND: case ByteCode.IASTORE: case
	 * ByteCode.ICONST_0: case ByteCode.ICONST_1: case ByteCode.ICONST_2: case
	 * ByteCode.ICONST_3: case ByteCode.ICONST_4: case ByteCode.ICONST_5: case
	 * ByteCode.ICONST_M1: case ByteCode.IDIV: case ByteCode.ILOAD_0: case
	 * ByteCode.ILOAD_1: case ByteCode.ILOAD_2: case ByteCode.ILOAD_3: case
	 * ByteCode.IMPDEP1: case ByteCode.IMPDEP2: case ByteCode.IMUL: case
	 * ByteCode.INEG: case ByteCode.IOR: case ByteCode.IREM: case
	 * ByteCode.IRETURN: case ByteCode.ISHL: case ByteCode.ISHR: case
	 ...
	 * ByteCode.IXOR: case ByteCode.L2D: case ByteCode.L2F: case ByteCode.L2I:
	 * case ByteCode.LADD: case ByteCode.LALOAD: case ByteCode.LAND: case
	 ...
	 * ByteCode.LXOR: case ByteCode.MONITORENTER: case ByteCode.MONITOREXIT:
	 * case ByteCode.NOP: case ByteCode.POP2: case ByteCode.POP: case
	 * ByteCode.RETURN: case ByteCode.SALOAD: case ByteCode.SASTORE: case
	 ...
	 * ByteCode.IFGE: case ByteCode.IFGT: case ByteCode.IFLE: case
	 * ByteCode.IFLT: case ByteCode.IFNE: case ByteCode.IFNONNULL: case
	 * ByteCode.IFNULL: case ByteCode.IF_ACMPEQ: case ByteCode.IF_ACMPNE: case
	 * ByteCode.IF_ICMPEQ: case ByteCode.IF_ICMPGE: case ByteCode.IF_ICMPGT:
	 * case ByteCode.IF_ICMPLE: case ByteCode.IF_ICMPLT: case
	 * ByteCode.IF_ICMPNE: case ByteCode.IINC: case ByteCode.INSTANCEOF: case
	 * ByteCode.INVOKEINTERFACE: case ByteCode.INVOKESPECIAL: case
	 * ByteCode.INVOKESTATIC: case ByteCode.INVOKEVIRTUAL: case ByteCode.JSR:
	 * case ByteCode.LDC2_W: case ByteCode.LDC_W: case ByteCode.NEW: case
	 * ByteCode.PUTFIELD: case ByteCode.PUTSTATIC: case ByteCode.SIPUSH: return
	 * 2;
	 * 
	 ...
	 * 
	 * A pc is the beginning of a super block if: - pc == 0 - it is the target
	 * of a branch instruction - it is the beginning of an exception handler -
	...
	static {
		...
		try {
			is = ClassFileWriter.class
					.getResourceAsStream("org/mozilla/classfile/ClassFileWriter.class");
			if (is == null) {
				is = ClassLoader
						.getSystemResourceAsStream("org/mozilla/classfile/ClassFileWriter.class");
			}
...
final class ConstantPool {
	...
	static final byte CONSTANT_Class = 7, CONSTANT_Fieldref = 9,
 ...
 * point and multiple exit points (therefore ending in an unconditional jump or
 * the end of the method). This is used to emulate OpenJDK's compiler, which
...
/**
 * Helper class for internal representations of type information. In most cases,
 * type information can be represented by a constant, but in some cases, a
 * payload is included. Despite the payload coming after the type tag in the
 * output, we store it in bits 8-23 for uniformity; the tag is always in bits
 * 0-7.
 */
final class TypeInfo {
	 ...
	 * INTEGER and DOUBLE cannot be merged and an exception will be thrown. The
	 * basic rules are:
	 * 
	 * - If the types are equal, simply return one. - If either type is TOP,
	 * return TOP. - If either type is NULL, return the other type. - If both
	 * types are objects, find the lowest common ancestor in the class
	 * hierarchy.
	 * 
	 ...
	 * of a full object-object merge, which would need to load the current class
	 * reflectively.
	 */
	static int merge(int current, int incoming, ConstantPool pool) {
		...
		if (current == incoming || (currentIsObject && incoming == NULL)) {
			...
			String currentlyGeneratedSuperName = (String) pool
					.getConstantData(4);
		...
		throw new IllegalArgumentException("bad merge attempt between "
				+ toString(current, pool) + " and " + toString(incoming, pool));
	}
