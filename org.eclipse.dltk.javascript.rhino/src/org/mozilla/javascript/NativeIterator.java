...
public final class NativeIterator extends IdScriptableObject {
	...
	/**
	 * Get the value of the "StopIteration" object. Note that this value is
	 * stored in the top-level scope using "associateValue" so the value can
	 * still be found even if a script overwrites or deletes the global
	 * "StopIteration" property.
	 * 
	 * @param scope
	 *            a scope whose parent chain reaches a top-level scope
	...
	static class StopIteration extends NativeObject {
		...
		/*
		 * StopIteration has custom instanceof behavior since it doesn't have a
		 * constructor.
		 */
		@Override
	...
	private static Object jsConstructor(Context cx, Scriptable scope,
		...
		if (thisObj != null) {
			...
			// iteration methods
			Iterator<?> iterator = VMBridge.instance.getJavaIterator(cx, scope,
					obj);
			if (iterator != null) {
	...
	private static final int Id_constructor = 1, Id_next = 2,
			Id___iterator__ = 3, MAX_PROTOTYPE_ID = 3;
