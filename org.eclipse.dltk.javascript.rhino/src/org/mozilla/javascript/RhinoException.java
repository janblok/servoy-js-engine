...
public abstract class RhinoException extends RuntimeException {
	 ...
	 * 
	 * @param sourceName
	 *            the uri of the script source responsible for the error. It
	 ...
	 * 
	 * @param lineSource
	 *            the text of the source line responsible for the error. It
	 *            should not be <tt>null</tt>.
	 * 
	 * @throws IllegalStateException
	 *             if the method is called more then once.
	 ...
	 * optimization is enabled, this includes java stack elements whose source
	 * and method names suggest they have been generated by the Rhino script
	 * compiler.
	 * 
	 ...
	 * {@link ScriptStackElement}s. If optimization is enabled, this includes
	 * java stack elements whose source and method names suggest they have been
	...
	public ScriptStackElement[] getScriptStack() {
		...
		for (StackTraceElement e : stack) {
			...
			if (e.getMethodName().startsWith("_c_") && e.getLineNumber() > -1
					&& fileName != null && !fileName.endsWith(".java")) {
				String methodName = e.getMethodName();
				Matcher match = pattern.matcher(methodName);
				// the method representing the main script is always
				// "_c_script_0" -
				// at least we hope so
				methodName = !"_c_script_0".equals(methodName) && match.find() ? match
						.group(1) : null;
				list.add(new ScriptStackElement(fileName, methodName, e
						.getLineNumber()));
			} else if ("org.mozilla.javascript.Interpreter".equals(e
					.getClassName())
	...
	/**
	 * Returns true if subclasses of <code>RhinoException</code> use the
	 * Mozilla/Firefox style of rendering script stacks (
	 * <code>functionName()@fileName:lineNumber</code>) instead of Rhino's own
	...
	/**
	 * Tell subclasses of <code>RhinoException</code> whether to use the
	 * Mozilla/Firefox style of rendering script stacks (
	 * <code>functionName()@fileName:lineNumber</code>) instead of Rhino's own
	 * Java-inspired format (
	 * <code>    at fileName:lineNumber (functionName)</code>)
	 * 
	 * @param flag
	 *            whether to render stacks in Mozilla/Firefox style
	 * @see ScriptStackElement
	 * @since 1.7R3
	 */
