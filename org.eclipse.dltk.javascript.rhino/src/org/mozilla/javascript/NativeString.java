...
/**
 * This class implements the String native object. See ECMA 15.5. String methods
 * for dealing with regular expressions are ported directly from C. Latest port
 * is from version 1.40.12.19 in the JSFUN13_BRANCH.
 * 
 * @author Mike McCabe
 * @author Norris Boyd
 */
final class NativeString extends IdScriptableObject implements Wrapper
{
	...
	private static NativeString protoType;
	...
	{
		if (s.equals("length")) { return instanceIdInfo(DONTENUM | READONLY | PERMANENT, Id_length); }
	...
	{
		if (id == Id_length) { return ScriptRuntime.wrapInt(string.length()); }
	...
	{
		if (!f.hasTag(STRING_TAG)) { return super.execIdCall(f, cx, scope, thisObj, args); }
		int id = f.methodId();
		switch (id)
		{
			...
			case Id_localeCompare:
			{
				...
				return ScriptRuntime.wrapNumber(collator.compare(ScriptRuntime.toString(thisObj), ScriptRuntime.toString(
						args, 0)));
			}
			case Id_toLocaleLowerCase:
			{
	...
	{
		if (0 <= index && index < string.length()) { return string.substring(index, index + 1); }
		return super.get(index, start);
	}

	public void put(int index, Scriptable start, Object value)
	{
		if (0 <= index && index < string.length()) { return; }
	 ...
	 * offset ip and looking either for the given separator substring, or for the
	 * next re match. ip and matchlen must be reference variables (assumed to be
	 * arrays of length 1) so they can be updated in the leading whitespace or re
	 * case. Return -1 on end of string, >= 0 for a valid index of the next
	 * separator occurrence if found, or the string length if no separator is
	 * found.
	 */
	private static int find_split(Context cx, Scriptable scope, String target, String separator, int version,
			RegExpProxy reProxy, Scriptable re, int[] ip, int[] matchlen, boolean[] matched, String[][] parensp)
	{
		int i = ip[0];
		...
		/*
		 * Stop if past end of string. If at end of string, we will return target
		 * length, so that "ab,".split(',') => new Array("ab", "") and the
		 * resulting array converts back to the string "ab," for symmetry. NB:
		 * This differs from perl, which drops the trailing empty substring if the
		 * LIMIT argument is omitted.
		 */
		if (i > length)
			return -1;
		 ...
		 * get stuck in a loop.
		 */
		if (re != null) { return reProxy.find_split(cx, scope, target, separator, re, ip, matchlen, matched, parensp); }
		 ...
		 * string into an empty substring. For 1.2 compatibility, at the end of
		 * the string, we return the length as the result, and set the separator
		 * length to 1 -- this allows the caller to include an additional null
		 * string at the end of the substring list.
		 */
		...
		/*
		 * Punt to j.l.s.indexOf; return target length if separator is not found.
		 */
		if (ip[0] >= length)
			return length;
	...
	Id_constructor = 1, Id_toString = 2, Id_toSource = 3, Id_valueOf = 4, Id_charAt = 5, Id_charCodeAt = 6,
			Id_indexOf = 7, Id_lastIndexOf = 8, Id_split = 9, Id_substring = 10, Id_toLowerCase = 11, Id_toUpperCase = 12,
			Id_substr = 13, Id_concat = 14, Id_slice = 15, Id_bold = 16, Id_italics = 17, Id_fixed = 18, Id_strike = 19,
			Id_small = 20, Id_big = 21, Id_blink = 22, Id_sup = 23, Id_sub = 24, Id_fontsize = 25, Id_fontcolor = 26,
			Id_link = 27, Id_anchor = 28, Id_equals = 29, Id_equalsIgnoreCase = 30, Id_match = 31, Id_search = 32,
			Id_replace = 33, Id_localeCompare = 34, Id_toLocaleLowerCase = 35, Id_toLocaleUpperCase = 36,
			MAX_PROTOTYPE_ID = 36;
