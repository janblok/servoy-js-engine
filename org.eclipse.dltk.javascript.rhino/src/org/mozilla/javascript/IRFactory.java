...
final class IRFactory
{
	 ...
	 * @param catchCond
	 *           the condition under which to catch the exception. May be null if
	...
	Node initFunction(FunctionNode fnNode, int functionIndex, Node statements, int functionType)
	{
		...
		if (functionType == FunctionNode.FUNCTION_EXPRESSION)
		{
			...
			if (name != null && name.length() != 0 && !fnNode.hasParamOrVar(name))
			{
				...
				Node setFn = new Node(Token.EXPR_VOID, new Node(Token.SETNAME, Node.newString(Token.BINDNAME, name),
	...
	/**
	 * Try/Catch/Finally The IRFactory tries to express as much as possible in
	 * the tree; the responsibilities remaining for Codegen are to add the Java
	 * handlers: (Either (but not both) of TARGET and FINALLY might not be
	 * defined) - a catch handler for javascript exceptions that unwraps the
	 * exception onto the stack and GOTOes to the catch target - a finally
	 * handler ... and a goto to GOTO around these handlers.
	 */
	Node createTryCatchFinally(Node tryBlock, Node catchBlocks, Node finallyBlock, int lineno)
	{
		...
		// short circuit
		if (tryBlock.getType() == Token.BLOCK && !tryBlock.hasChildren() && !hasFinally) { return tryBlock; }
	...
	Node createCondExpr(Node cond, Node ifTrue, Node ifFalse)
	{
		...
		else if (condStatus == ALWAYS_FALSE_BOOLEAN) { return ifFalse; }
	...
	Node createPropertyGet(Node target, String namespace, String name, int memberTypeFlags)
	{
		...
		{
			if (target == null) { return createName(name); }
	...
	private void checkActivationName(String name, int token)
	{
		if (parser.insideFunction())
		{
			...
			if ("arguments".equals(name)
					|| (parser.compilerEnv.activationNames != null && parser.compilerEnv.activationNames.containsKey(name)))
			{
				activation = true;
			}
