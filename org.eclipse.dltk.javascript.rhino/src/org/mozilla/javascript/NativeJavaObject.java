...
public class NativeJavaObject implements Scriptable, Wrapper, Serializable
{
	...
	{
		if (prototype == null && javaObject instanceof String) { return ScriptableObject.getClassPrototype(parent,
				"String"); }
		return prototype;
	}
	...
	/**
	 * Derive a ranking based on how "natural" the conversion is. The special
	 * value CONVERSION_NONE means no conversion is possible, and
	 * CONVERSION_NONTRIVIAL signals that more type conformance testing is
	 * required. Based on <a
	 * href="http://www.mozilla.org/js/liveconnect/lc3_method_overloading.html">
	...
	static int getConversionWeight(Object fromObj, Class to)
	{
		...
		switch (fromCode)
		{
			...
			case JSTYPE_UNDEFINED:
				if (to == ScriptRuntime.StringClass || to == ScriptRuntime.ObjectClass) { return 1; }
				break;

			case JSTYPE_NULL:
				if (!to.isPrimitive()) { return 1; }
				break;
			...
			case JSTYPE_BOOLEAN:
				...
				else if (to == ScriptRuntime.StringClass) { return 4; }
				break;
			...
			case JSTYPE_NUMBER:
				if (to.isPrimitive())
				{
					...
					else if (to != Boolean.TYPE) { return 1 + getSizeRank(to); }
				}
				else
				{
			...
			case JSTYPE_STRING:
				...
				else if (to.isPrimitive())
				{
					...
					else if (to != Boolean.TYPE) { return 4; }
				}
				break;
			...
			case JSTYPE_OBJECT:
				...
				else if (to.isInterface())
				{
					if (fromObj instanceof Function)
					{
						...
						if (to.getMethods().length == 1) { return 1; }
					}
					return 11;
				}
				else if (to.isPrimitive() && to != Boolean.TYPE) { return 3 + getSizeRank(to); }
				break;
		}
	...
	{
		if (value != null && value.getClass() == type) { return value; }
		...
		switch (getJSTypeCode(value))
		{
			...
			case JSTYPE_NUMBER:
				...
				else if (type == ScriptRuntime.BooleanClass || type == Boolean.TYPE)
				{
					return ScriptRuntime.toBoolean(value) ? Boolean.TRUE : Boolean.FALSE;
				}
				else
				{
					reportConversionError(value, type);
				...
				else
				{
					...
					else if (type.isArray() && value.getClass().isArray())
					{
						...
						for (int i = 0; i < length; ++i)
						{
							try
							{
								Array.set(result, i, coerceType(arrayType, Array.get(value,i)));
							}
	...
	private static Object coerceToNumber(Class type, Object value)
	{
		...
		{
			if (valueClass == ScriptRuntime.CharacterClass) { return value; }
			return new Character((char) toInteger(value, ScriptRuntime.CharacterClass, Character.MIN_VALUE,
					Character.MAX_VALUE));
		}

		// Double, Float
		if (type == ScriptRuntime.ObjectClass || type == ScriptRuntime.DoubleClass || type == Double.TYPE) { return valueClass == ScriptRuntime.DoubleClass ? value
		...
		if (type == ScriptRuntime.LongClass || type == Long.TYPE)
		{
			...
			{
				/*
				 * Long values cannot be expressed exactly in doubles. We thus use
				 * the largest and smallest double value that has a value
				 * expressible as a long value. We build these numerical values from
				 * their hexidecimal representations to avoid any problems caused by
				 * attempting to parse a decimal representation.
				 */
	...
	static void reportConversionError(Object value, Class type)
	{
		...
		throw Context.reportRuntimeError2("msg.conversion.not.allowed", String.valueOf(value), JavaMembers
				.javaSignature(type));
	}
	...
	private void writeObject(ObjectOutputStream out) throws IOException
	{
		...
		if (isAdapter)
		{
			if (adapter_writeAdapterObject == null) { throw new IOException(); }
