 ...
 * and caches the loaded modules. It softly references the loaded modules' Rhino
 * {@link Script} objects, thus a module once loaded can become eligible for
 * garbage collection if it is otherwise unused under memory pressure. Instances
 * of this class are thread safe.
 * 
 * @author Attila Szegedi
 * @version $Id: SoftCachingModuleScriptProvider.java,v 1.3 2011/04/07 20:26:12
 *          hannes%helma.at Exp $
 */
public class SoftCachingModuleScriptProvider extends
		CachingModuleScriptProviderBase {
	private static final long serialVersionUID = 1L;

	private transient ReferenceQueue<Script> scriptRefQueue = new ReferenceQueue<Script>();

	private transient ConcurrentMap<String, ScriptReference> scripts = new ConcurrentHashMap<String, ScriptReference>(
			16, .75f, getConcurrencyLevel());
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		final Map<String, CachedModuleScript> serScripts = (Map) in
				.readObject();
		for (Map.Entry<String, CachedModuleScript> entry : serScripts
				.entrySet()) {
