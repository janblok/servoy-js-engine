...
/**
 * Abstract base class that implements caching of loaded module scripts. It uses
 * a {@link ModuleSourceProvider} to obtain the source text of the scripts. It
 * supports a cache revalidation mechanism based on validator objects returned
 * from the {@link ModuleSourceProvider}. Instances of this class and its
 * subclasses are thread safe (and written to perform decently under concurrent
 * access).
 * 
 * @author Attila Szegedi
 * @version $Id: CachingModuleScriptProviderBase.java,v 1.3 2011/04/07 20:26:12
 *          hannes%helma.at Exp $
 */
public abstract class CachingModuleScriptProviderBase implements
		ModuleScriptProvider, Serializable {
	...
	public ModuleScript getModuleScript(Context cx, String moduleId,
		...
		final ModuleSource moduleSource = (moduleUri == null) ? moduleSourceProvider
				.loadSource(moduleId, paths, validator1) : moduleSourceProvider
				.loadSource(moduleUri, validator1);
			...
			synchronized (loadLocks[(idHash >>> loadLockShift) & loadLockMask]) {
				...
				final ModuleScript moduleScript = new ModuleScript(
						cx.compileReader(reader, sourceUri.toString(), 1,
								moduleSource.getSecurityDomain()), sourceUri,
	 ...
	 * 
	 * @author Attila Szegedi
	 * @version $Id: CachingModuleScriptProviderBase.java,v 1.3 2011/04/07
	...
	public static class CachedModuleScript {
		 ...
		 * 
		 * @param moduleScript
		 *            the module script itself
		 * @param validator
		 *            a validator for the moduleScript's source text entity.
		 */
