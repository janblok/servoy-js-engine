 ...
 * @version $Id: ModuleSourceProvider.java,v 1.3 2011/04/07 20:26:12
 *          hannes%helma.at Exp $
 */
public interface ModuleSourceProvider {
	/**
	 * A special return value for
	 * {@link #loadSource(String, Scriptable, Object)} and
	 * {@link #loadSource(URI, Object)} that signifies that the cached
	 ...
	 * @param validator
	 *            a validator for an existing loaded and cached module. This
	 *            will either be null, or an object that this source provider
	 *            returned earlier as part of a {@link ModuleSource}. It can be
	 *            used to validate the existing cached module and avoid
	 *            reloading it.
	 * @return a script representing the code of the module. Null should be
	 *         returned if the script is not found. {@link #NOT_MODIFIED} should
	 *         be returned if the passed validator validates the current
	 *         representation of the module (the currently cached module
	 *         script).
	 * @throws IOException
	 *             if there was an I/O problem reading the script
	...
	/**
	 * Returns the script source of the requested module from the given URI. The
	 * URI is absolute but does not contain a file name extension such as ".js",
	 * which may be specific to the ModuleSourceProvider implementation.
	 * <p>
	 ...
	 * that this source provider implements), then {@link #NOT_MODIFIED} should
	 * be returned. Otherwise, it should return a {@link ModuleSource} object
	 * with the actual source text of the module, preferrably a validator for
	 * it, and a security domain, where applicable.
	 * 
	 * @param uri
