...
class DToA {
	...
	static int JS_dtoa(double d, int mode, boolean biasUp, int ndigits,
		...
		if ((word0(d) & Exp_mask) == Exp_mask) {
			/* Infinity or NaN */
			buf.append(((word1(d) == 0) && ((word0(d) & Frac_mask) == 0)) ? "Infinity"
					: "NaN");
			return 9999;
		}
		if (d == 0) {
		 ...
		 * f.
		 */
		ds = (d2 - 1.5) * 0.289529654602168 + 0.1760912590558 + i
				* 0.301029995663981;
		k = (int) ds;
		if (ds < 0.0 && ds != k)
			k--; /* want k = floor(ds) */
		...
		if (leftright) {
			if (mode < 2) {
				i = (denorm) ? be[0] + (Bias + (P - 1) - 1 + 1) : 1 + P
						- bbits[0];
				/*
	...
	static void JS_dtostr(StringBuffer buffer, int mode, int precision, double d) {
		int decPt; /*
					 * Position of decimal point relative to first digit
					 * returned by JS_dtoa
					 */
