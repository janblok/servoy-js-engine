...
class DToA
{
	...
	DTOSTR_FIXED = 2, /*
							 * Round to <precision> digits after the decimal point;
							 * exponential if number is large
							 */
	DTOSTR_EXPONENTIAL = 3, /*
									 * Always exponential format; <precision> significant
									 * digits
									 */
	DTOSTR_PRECISION = 4; /*
	...
	static String JS_dtobasestr(int base, double d)
	{
		...
		else
		{
			...
			// JS_ASSERT(e < 0);
			/*
			 * At this point df = b * 2^e. e must be less than zero because 0 < df <
			 * 1.
			 */
			...
			s = s.shiftLeft(s2);
			/*
			 * At this point we have the following: s = 2^s2; 1 > df = b/2^s2 > 0;
			 * (d - prevDouble(d))/2 = mlo/2^s2; (nextDouble(d) - d)/2 = mhi/2^s2.
			 */
			...
			do
			{
				...
				else if (j < 0 || (j == 0 && ((word1 & 1) == 0)))
				{
					if (j1 > 0)
					{
						 ...
						 * significant digit. Use whichever would produce an output
						 * value closer to d.
						 */
						b = b.shiftLeft(1);
						j1 = b.compareTo(s);
						if (j1 > 0) /*
										 * The even test (|| (j1 == 0 && (digit & 1))) is
										 * not here because it messes up odd base output
	 ...
	 * Rather than iterating, we use a simple numeric overestimate to determine k =
	 * floor(log10(d)). We scale relevant quantities using O(log2(k)) rather than
	 * O(k) multiplications. 2. For some modes > 2 (corresponding to ecvt and
	 * fcvt), we don't try to generate digits strictly left to right. Instead, we
	 * compute with fewer bits and propagate the carry if necessary when rounding
	 * the final digit up. This is often faster. 3. Under the assumption that
	 * input will be rounded nearest, mode 0 renders 1e23 as 1e23 rather than
	 * 9.999999999999999e22. That is, we allow equality in stopping tests when
	 * the round-nearest rule will give the same floating-point value as would
	 * satisfaction of the stopping test with strict inequality. 4. We remove
	 ...
	 * cannot guarantee that the floating-point calculation has given the
	 * correctly rounded result. For k requested digits and "uniformly"
	 * distributed input, the probability is something like 10^(k-15) that we
	 * must resort to the Long calculation.
	 */
	 ...
	 * bufsize should be two greater than the maximum number of output characters
	 * expected.
	 */
	static int JS_dtoa(double d, int mode, boolean biasUp, int ndigits, boolean[] sign, StringBuffer buf)
	{
		 ...
		 * or NaN, then *decpt is set to 9999. mode: 0 ==> shortest string that
		 * yields d when read in and rounded to nearest. 1 ==> like 0, but with
		 * Steele & White stopping rule; e.g. with IEEE P754 arithmetic , mode 0
		 * gives 1e23 whereas mode 1 gives 9.999999999999999e22. 2 ==>
		 * max(1,ndigits) significant digits. This gives a return value similar to
		 * that of ecvt, except that trailing zeros are suppressed. 3 ==> through
		 * ndigits past the decimal point. This gives a return value similar to
		 * that from fcvt, except that trailing zeros are suppressed, and ndigits
		 * can be negative. 4-9 should give the same return values as 2-3, i.e., 4 <=
		 * mode <= 9 ==> same return as mode 2 + (mode & 1). These modes are
		 * mainly for debugging; often they run slower but sometimes faster than
		 * modes 2-3. 4,5,8,9 ==> left-to-right digit generation. 6-9 ==> don't
		 * try fast floating-point estimate (if applicable). Values of mode other
		 * than 0-9 are treated as mode 0. Sufficient space is allocated to the
		...
		if ((i = (word0(d) >>> Exp_shift1 & (Exp_mask >> Exp_shift1))) != 0)
		{
			 ...
			 * (i - Bias) by 0.301029995663981; since |i - Bias| <= 1077, and 1077 *
			 * 0.30103 * 2^-52 ~=~ 7.2e-14, adding 1e-13 to the constant term more
			 * than suffices. Hence we adjust the constant term to 0.1760912590558.
			 * (We could get a more accurate k by invoking log10, but this is
		...
		/*
		 * At this point floor(log10(d)) <= k <= floor(log10(d))+1. If k_check is
		 * zero, we're guaranteed that k = floor(log10(d)).
		 */
		j = bbits[0] - i - 1;
		...
		/*
		 * At this point d/10^k = (b * 2^b2 * 5^b5) / (2^s2 * 5^s5), where b is an
		 * odd integer, b2 >= 0, b5 >= 0, s2 >= 0, and s5 >= 0.
		 */
		if (mode < 0 || mode > 9)
			mode = 0;
		try_quick = true;
		...
		if (leftright)
		{
			if (mode < 2)
			{
				 ...
				 * Thus, (2^m2 * 5^m5) / (2^(s2+i) * 5^s5) = (1/2 lsb of d)/10^k.
				 */
			}
			else
			{
			 ...
			 * mode >= 2) or input (when mode < 2) significant digit, divided by
			 * 10^k.
			 */
		}
		/*
		 ...
		 * 2^s2) = one-half of last printed or input significant digit, divided by
		 * 10^k.
		 */
		...
		if (mode < 2)
		{
			if ((word1(d) == 0) && ((word0(d) & Bndry_mask) == 0) && ((word0(d) & (Exp_mask & Exp_mask << 1)) != 0))
			{
				/*
				 * The special case. Here we want to be within a quarter of the last
				 * input significant digit instead of one half of it when the
				 * decimal output string's value is less than d.
				 */
				b2 += Log2P;
				s2 += Log2P;
				spec_case = true;
		 ...
		 * compute leading 28 bits of S once and for all and pass them and a shift
		 * to quorem, so it can do shifts and ors to compute the numerator for q.
		 */
		...
		if (ilim <= 0 && mode > 2)
		{
			 ...
			 * output in this mode. Output either zero or the minimum nonzero
			 * output depending on which is closer to d.
			 */
			if ((ilim < 0) || ((i = b.compareTo(S = S.multiply(BigInteger.valueOf(5)))) < 0) || ((i == 0 && !biasUp)))
			{
				/*
				 * Always emit at least one digit. If the number appears to be zero
				 * using the current mode, then emit one '0' digit and set decpt to
				 * 1.
				 */
				/*
		...
		if (leftright)
		{
			...
			/*
			 * mlo/S = maximum acceptable error, divided by 10^k, if the output is
			 * less than d.
			 */
			/*
			 * mhi/S = maximum acceptable error, divided by 10^k, if the output is
			 * greater than d.
			 */
			...
			for (i = 1;; i++)
			{
				...
				if ((j < 0) || ((j == 0) && (mode == 0) && ((word1(d) & 1) == 0)))
				{
					if (j1 > 0)
					{
						/*
						 * Either dig or dig+1 would work here as the least
						 * significant decimal digit. Use whichever would produce a
						 * decimal value closer to d.
						 */
						b = b.shiftLeft(1);
						j1 = b.compareTo(S);
	...
	static void JS_dtostr(StringBuffer buffer, int mode, int precision, double d)
	{
		...
		if (mode == DTOSTR_FIXED && (d >= 1e21 || d <= -1e21))
			mode = DTOSTR_STANDARD; /*
											 * Change mode here rather than below because
											 * the buffer may not be large enough to hold a
											 * large integer.
											 */
