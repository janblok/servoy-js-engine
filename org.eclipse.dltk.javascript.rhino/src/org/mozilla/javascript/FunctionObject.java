...
public class FunctionObject extends BaseFunction
{
	 ...
	 * java.lang.reflect.Method or a java.lang.reflect.Constructor and must match
	 * one of two forms.
	 * <p>
	 ...
	 * is not supported because the double representation of a long (which is the
	 * EMCA-mandated storage type for Numbers) may lose precision. If the member
	 * is a Method, the return value must be void or one of the types allowed for
	 * parameters.
	 * <p>
	 ...
	 * constructor, the member must be a static Method with parameters (Context
	 * cx, Scriptable thisObj, Object[] args, Function funObj)
	 * 
	 * </pre>
	 * <pre>
	 * 
	 ...
	 * as a function, will convert to boolean rather than creating a new object.)
	 * <p>
	 * 
	 * @param name
	...
	public FunctionObject(String name, Member methodOrConstructor, Scriptable scope)
	{
		...
		if (arity == 4 && (types[1].isArray() || types[2].isArray()))
		{
			...
			if (types[1].isArray())
			{
				if (!isStatic || types[0] != ScriptRuntime.ContextClass
						...
						|| types[2] != ScriptRuntime.FunctionClass || types[3] != Boolean.TYPE) { throw Context
						.reportRuntimeError1("msg.varargs.ctor", methodName); }
			...
			else
			{
				if (!isStatic || types[0] != ScriptRuntime.ContextClass || types[1] != ScriptRuntime.ScriptableClass
						...
						|| types[3] != ScriptRuntime.FunctionClass) { throw Context.reportRuntimeError1("msg.varargs.fun",
						methodName); }
		...
		else
		{
			...
			if (!ScriptRuntime.ScriptableClass.isAssignableFrom(ctorType)) { throw Context.reportRuntimeError1(
	...
	// {
	// obj = convertArg(this, obj, types[0].getComponentType());
	...
	static Method findSingleMethod(Method[] methods, String name)
	{
		...
		for (int i = 0, N = methods.length; i != N; ++i)
		{
			...
			{
				if (found != null) { throw Context.reportRuntimeError2("msg.no.overload", name, method.getDeclaringClass()
						.getName()); }
				found = method;
			}
		}
	...
	static Method[] getMethodList(Class clazz)
	{
		...
		{
			if (sawSecurityException ? methods[i].getDeclaringClass() != clazz : !Modifier.isPublic(methods[i]
					.getModifiers()))
			{
				methods[i] = null;
			}
	 ...
	 * and setPrototype with appropriate values. Then adds the function object as
	 * a property of the given scope, using <code>prototype.getClassName()</code>
	...
	void initAsConstructor(Scriptable scope, Scriptable prototype)
	{
		...
		defineProperty(prototype, "constructor", this, ScriptableObject.DONTENUM | ScriptableObject.PERMANENT
	...
	public static Object convertArg(Context cx, Scriptable scope, Object arg, Class desired)
	{
		...
		if (tag == JAVA_UNSUPPORTED_TYPE) { throw Context.reportRuntimeError1("msg.cant.convert", desired.getName()); }
	...
	public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args)
	{
		...
		if (parmsLength < 0)
		{
			...
			else
			{
				...
				result = (member.isCtor()) ? member.newInstance(invokeArgs) : member.invoke(thisObj, invokeArgs);
			}
		...
		else
		{
			...
			Object[] invokeArgs;
			// TODO var arrays
			// if(parmsLength == 1 && typeTags[0].isArray())
			// {
			// for (int j = 0; j < args.length; j++)
			// {
			// args[j] = unwrap(args[j]);
			// }
	...
	{
		if (member.isCtor() || parmsLength == VARARGS_CTOR) { return null; }
		Scriptable result;
		try
		{
