...
/**
 * This class represents the runtime context of an executing script. Before
 * executing a script, an instance of Context must be created and associated
 * with the thread that will be executing the script. The Context will be used
 * to store information about the executing of the script such as the call
 * stack. Contexts are associated with the current thread using the
...
{
	/**
	 * Language versions. All integral values are reserved for future version
	 * numbers.
	 */
	 ...
	 * identifiers but warn about this usage. By default {@link #hasFeature(int)}
	 * returns false.
	 */
	 ...
	 * treated specially. If <tt>hasFeature(FEATURE_PARENT_PROTO_PROPERTIES)</tt>
	 * returns true, treat <tt>__parent__</tt> and <tt>__proto__</tt> as
	 * special properties.
	 * <p>
	 ...
	 * <tt>x.__proto__ = y</tt> changes the prototype chain of the object
	 * <tt>x</tt> to point to <tt>y</tt>, <tt>x["__proto__"] = y</tt>
	 * simply assigns a new value to the property <tt>__proto__</tt> in
	 * <tt>x</tt> even when the feature is on. By default
	 * {@link #hasFeature(int)} returns true.
	 */
	 ...
	 * <p>
	 * By default {@link #hasFeature(int)} returns true if the current JS version
	 * is set to {@link #VERSION_DEFAULT} or is at least {@link #VERSION_1_6}.
	 * 
	 * @since 1.6 Release 1
	 */
	 ...
	 * have to explicitly pass them as the second and third argument to the Error
	 * constructor. Note that neither behaviour is fully ECMA 262 compliant (as
	 * 262 doesn't specify a three-arg constructor), but keeping the feature off
	 * results in Error objects that don't have additional non-ECMA properties
	 * when constructed using the ECMA-defined single-arg constructor and is thus
	 * desirable if a stricter ECMA compliance is desired, specifically adherence
	 * to the point 15.11.5. of the standard.
	 * <p>
	 * By default {@link #hasFeature(int)} returns false.
	 * 
	 * @since 1.6 Release 6
	...
	/**
	 * Create a new Context. Note that the Context must be associated with a
	 * thread before it can be used to execute a script.
	 * 
	 * @see #enter()
	...
	/**
	 * Get the current Context. The current Context is per-thread; this method
	 * looks up the Context associated with the current thread.
	 * <p>
	 * 
	...
	static final Context enter(Context cx, ContextFactory factory)
	{
		...
		else
		{
			...
			if (cx.enterCount != 0 || cx.factory != null) { throw new IllegalStateException(); }
	...
	/**
	 * Exit a block of code requiring a Context. Calling <code>exit()</code>
	 * will remove the association between the current thread and a Context if
	 * the prior call to <code>enter()</code> on this thread newly associated a
	...
	static void exit(ContextFactory factory)
	{
		...
		if (cx == null) { throw new IllegalStateException("Calling Context.exit without previous Context.enter"); }
		if (cx.factory != null)
		{
			// Context with associated factory will be released
			// automatically and does not need to change enterCount
	 ...
	 * @see ContextFactory#call(ContextAction)
	 */
	public static Object call(ContextFactory factory, Callable callable, Scriptable scope, Scriptable thisObj,
			Object[] args)
	{
		if (factory == null)
		{
	...
	private static Context prepareNewContext(ContextFactory factory, Object contextHelper)
	{
		...
		if (cx.factory != null || cx.enterCount != 0) { throw new IllegalStateException(
	...
	{
		if (errorReporter == null) { return DefaultErrorReporter.instance; }
	 ...
	 * @param lineOffset
	 *           the offset into lineSource where problem was detected
	 * @return a runtime exception that will be thrown to terminate the execution
	 *         of the script
	 * @see org.mozilla.javascript.ErrorReporter
	 */
	...
	/**
	 * Initialize the standard objects. Creates instances of the standard objects
	 * and their constructors (Object, String, Number, Date, etc.), setting up
	 * 'scope' to act as a global object as in ECMA 15.1.
	 * <p>
	...
	/**
	 * Evaluate a JavaScript source string. The provided source name and line
	 * number are used for error messages and for producing debug information.
	 * 
	 * @param scope
	...
	/**
	 * Evaluate a reader as JavaScript source. All characters of the reader are
	 * consumed.
	 * 
	 * @param scope
	 *           the scope to execute in
	...
	final Script compileString(String source, Interpreter compiler, ErrorReporter compilationErrorReporter,
	...
	final Function compileFunction(Scriptable scope, String source, Interpreter compiler,
	 ...
	 * canonical source, omitting the function header and trailing brace. Returns
	 * '[native code]' if no decompilation information is available.
	 * 
	 * @param fun
	 *           the JavaScript function to decompile
	 * @param indent
	...
	/**
	 * Create a new JavaScript object by executing the named constructor. The
	 * call <code>newObject(scope, "Foo")</code> is equivalent to evaluating
	 * "new Foo()".
	 * 
	 * @param scope
	 *           the scope to search for the constructor and to evaluate against
	 ...
	 * the number is converted Uint32 value as defined in Ecma 9.6 and Java array
	 * of that size is allocated. The array is initialized with the values
	 * obtained by calling get() on object for each value of i in [0,length-1].
	 * If there is not a defined value for a property the Undefined value is used
	 * to initialize the corresponding element in the array. The Java array is
	 * then returned. If the object doesn't define a length property or it is not
	 * a number, empty array is returned.
	 * 
	 * @param object
	...
	public static RuntimeException throwAsScriptRuntimeEx(Throwable e)
	{
		...
		if (e instanceof RhinoException) { throw (RhinoException) e; }
	 ...
	 * time performance for runtime performance. The optimizer level can't be set
	 * greater than -1 if the optimizer package doesn't exist at run time.
	 * 
	...
	{
		if (isValidOptimizationLevel(optimizationLevel)) { return; }
	 ...
	 * Defaults to Integer.MAX_VALUE. The setting only has effect for interpreted
	 * functions (those compiled with optimization level set to -1). As the
	 * interpreter doesn't use the Java stack but rather manages its own stack in
	 * the heap memory, a runaway recursion in interpreted code would eventually
	 * consume all available memory and cause OutOfMemoryError instead of a
	 * StackOverflowError limited to only a single thread. This setting helps
	 ...
	 * the interpreter will throw an EvaluatorException in the script. Defaults
	 * to Integer.MAX_VALUE. The setting only has effect for interpreted
	 * functions (those compiled with optimization level set to -1). As the
	 * interpreter doesn't use the Java stack but rather manages its own stack in
	 * the heap memory, a runaway recursion in interpreted code would eventually
	 * consume all available memory and cause OutOfMemoryError instead of a
	...
	public final void setMaximumInterpreterStackDepth(int max)
	{
		if (sealed)
			onSealedMutation();
		if (optimizationLevel != -1) { throw new IllegalStateException(
				"Cannot set maximumInterpreterStackDepth when optimizationLevel != -1"); }
		if (max < 1) { throw new IllegalArgumentException("Cannot set maximumInterpreterStackDepth to less than 1"); }
	...
	public final void setSecurityController(SecurityController controller)
	{
		...
		if (controller == null)
			throw new IllegalArgumentException();
		if (securityController != null) { throw new SecurityException(
				"Can not overwrite existing SecurityController object"); }
		if (SecurityController.hasGlobal()) { throw new SecurityException(
	...
	public final void setClassShutter(ClassShutter shutter)
	{
		...
		if (shutter == null)
			throw new IllegalArgumentException();
		if (classShutter != null) { throw new SecurityException("Cannot overwrite existing " + "ClassShutter object"); }
	 ...
	 * disassociated from the thread the values cannot be retreived. Also, if
	...
	{
		if (script instanceof NativeFunction) { return ((NativeFunction) script).getDebuggableView(); }
		return null;
	}
	 ...
	 * considered experimental. The default implementation uses the
	 * implementation provided by this <code>Context</code>'s
	 * {@link ContextFactory}.
	 * 
	 ...
	 * <p>
	 * The instruction counting support is available only for interpreted scripts
	 * generated when the optimization level is set to -1.
	 * <p>
	 * The default implementation calls
	...
	public final void setApplicationClassLoader(ClassLoader loader)
	{
		...
		if (!Kit.testIfCanLoadRhinoClasses(loader)) { throw new IllegalArgumentException(
	...
	private Object compileImpl(Scriptable scope, Reader sourceReader, String sourceString, String sourceName,
			int lineno, Object securityDomain, boolean returnFunction, Interpreter compiler,
			ErrorReporter compilationErrorReporter) throws IOException
	{
		if (securityDomain != null && securityController == null) { throw new IllegalArgumentException(
	...
	static String getSourcePositionFromStack(int[] linep)
	{
		...
		if (cx == null)
			return null;
		if (cx.lastInterpreterFrame != null) { return Interpreter.getSourcePositionFromStack(cx, linep); }
		/**
		 * A bit of a hack, but the only way to get filename and line number from
		 * an enclosing frame.
		 */
	...
	public static void setDispatchingEventsEnabled(boolean b)
	{
		isDispatchingEventsEnabled = b;
	}

	public static boolean isDispatchingEventsEnabled()
	{
		return isDispatchingEventsEnabled;
	}
	...
	static boolean isDispatchingEventsEnabled = true;
