...
public final class Interpreter extends Icode implements Evaluator {
	...
	public Object compile(CompilerEnvirons compilerEnv, ScriptNode tree,
		...
		itsData = cgen
				.compile(compilerEnv, tree, encodedSource, returnFunction);
		return itsData;
	}
	...
	private static Object interpretLoop(Context cx, CallFrame frame,
		...
		StateLoop: for (;;) {
			withoutExceptions: try {
				...
				Loop: for (;;) {
					...
					jumplessRun: {
						...
						case Token.SHNE: {
							--stackTop;
							boolean valBln = shallowEquals(stack, sDbl,
									stackTop);
						...
						case Token.SETNAME: {
							...
							stack[stackTop] = op == Token.SETNAME ? ScriptRuntime
									.setName(lhs, rhs, cx, frame.scope,
											stringReg) : ScriptRuntime
									.strictSetName(lhs, rhs, cx, frame.scope,
											stringReg);
							continue Loop;
						}
						case Icode_SETCONST: {
						...
						case Token.GETPROPNOWARN: {
							...
							if (lhs == DBL_MRK)
								lhs = ScriptRuntime.wrapNumber(sDbl[stackTop]);
							stack[stackTop] = ScriptRuntime
									.getObjectPropNoWarn(lhs, stringReg, cx);
							continue Loop;
						}
						case Token.GETPROP: {
							Object lhs = stack[stackTop];
						...
						case Token.GETELEM: {
							...
							if (id != DBL_MRK) {
								value = ScriptRuntime.getObjectElem(lhs, id,
										cx, frame.scope);
							} else {
						...
						case Token.REF_CALL: {
							...
							stack[stackTop] = fun.call(
									...
									calleeScope,
									funThisObj,
									getArgsArray(stack, sDbl, stackTop + 2,
											indexReg));
						...
						case Token.ENUM_ID: {
							...
							stack[stackTop] = (op == Token.ENUM_NEXT) ? (Object) ScriptRuntime
									.enumNext(val) : (Object) ScriptRuntime
									.enumId(val, cx);
							continue Loop;
						}
			...
			if (generatorState != null
				...
				exState = cx.hasFeature(Context.FEATURE_ENHANCED_JAVA_ACCESS) ? EX_CATCH_STATE
						: EX_FINALLY_STATE;
	...
	private static CallFrame initFrameForNoSuchMethod(Context cx,
			...
			int stackTop, int op, Scriptable funThisObj,
	...
	private static Object freezeGenerator(Context cx, CallFrame frame,
		...
		return (frame.result != UniqueTag.DOUBLE_MARK) ? frame.result
	...
	private static CallFrame initFrameForApplyOrCall(Context cx,
			CallFrame frame, int indexReg, Object[] stack, double[] sDbl,
			int stackTop, int op, Scriptable calleeScope,
