...
public class Interpreter
{
	...
	// Stack: ... value1 -> ... value1 value1
			...
			Icode_REG_IND_C0 = -32, Icode_REG_IND_C1 = -33, Icode_REG_IND_C2 = -34,
			Icode_REG_IND_C3 = -35,
			Icode_REG_IND_C4 = -36, Icode_REG_IND_C5 = -37, Icode_REG_IND1 = -38,
			Icode_REG_IND2 = -39,
			Icode_REG_IND4 = -40,

			// Load string register to prepare for the following string operation
			Icode_REG_STR_C0 = -41, Icode_REG_STR_C1 = -42, Icode_REG_STR_C2 = -43,
			Icode_REG_STR_C3 = -44,
	...
	{
		if (!validBytecode(bytecode)) { throw new IllegalArgumentException(String.valueOf(bytecode)); }

		if (!Token.printICode) { return String.valueOf(bytecode); }

		if (validTokenCode(bytecode)) { return Token.name(bytecode); }
	...
	private void visitStatement(Node node)
	{
		...
		switch (type)
		{
			...
			case Token.FUNCTION:
			{
				...
				{
					if (fnType != FunctionNode.FUNCTION_STATEMENT) { throw Kit.codeBug(); }
				}
		...
		if (itsStackDepth != 0) { throw Kit.codeBug(); }
	}
	...
	private void visitExpression(Node node, int contextFlags)
	{
		...
		switch (type)
		{
			...
			case Token.FUNCTION:
			{
				...
				if (fn.getFunctionType() != FunctionNode.FUNCTION_EXPRESSION) { throw Kit.codeBug(); }
				addIndexOp(Icode_CLOSURE_EXPR, fnIndex);
				stackChange(1);
			}
				break;
	...
	{
		if (ex.interpreterStackInfo == null) { return null; }
	...
	{
		if (idata.encodedSource == null) { return null; }
	...
	{
		if (!ScriptRuntime.hasTopCall(cx)) { return ScriptRuntime.doTopCall(c, cx, scope, null, args); }
					...
					jumplessRun:
					{
						...
						switch (op)
						{
							...
							case Icode_STARTSUB:
								...
								{
									// Call from Icode_GOSUB: store return PC address in
									// the local
									indexReg += frame.localShift;
								...
								{
									// Call from exception handler: exception object is
									// already stored
									// in the local
							...
							case Icode_RETSUB:
							{
								...
								if (value != DBL_MRK)
								{
									// Invocation from exception handler, restore object
									// to rethrow
									throwable = value;
									break withoutExceptions;
								}
								...
								if (fun instanceof InterpretedFunction)
								{
									...
									if (frame.fnOrScript.securityDomain == ifun.securityDomain)
									{
										...
										if (op == Icode_TAIL_CALL)
										{
											...
											// including
											// StackOverflowException during innocent
											// looking
											// System.arraycopy may leave the current frame
											// data corrupted leading to undefined
											// behaviour
											// in the catch code bellow that unwinds JS
											// stack
											// on exceptions. Then there is issue about
											// frame release
											// end exceptions there.
											// To avoid frame allocation a released frame
											// can be cached for re-use which would also
											// benefit
											...
											// Release the current frame. See Bug #344501
											// to see why
											// it is being done here.
											// TODO: If using the graphical debugger, tail
											// call
											// optimization will create a "hole" in the
											// context stack.
											// The correct thing to do may be to disable
											// tail call
							...
							case Token.NEW:
							{
								...
								if (lhs instanceof InterpretedFunction)
								{
									...
									if (frame.fnOrScript.securityDomain == f.securityDomain)
									{
										...
										CallFrame calleeFrame = new CallFrame();
										initFrame(cx, frame.scope, newInstance, stack, sDbl, stackTop + 1, indexReg, f, frame,
												calleeFrame);
							...
							case Token.SETCONSTVAR:
								if (!frame.useActivation)
								{
									if ((varAttributes[indexReg] & ScriptableObject.READONLY) == 0) { throw Context
											.reportRuntimeError1("msg.var.redecl", frame.idata.argNames[indexReg]); }
							...
							case Token.CATCH_SCOPE:
							{
								...
								stack[indexReg] = ScriptRuntime.newCatchScope(caughtException, lastCatchScope, stringReg, cx,
										frame.scope);
								++frame.pc;
								continue Loop;
							}
			...
			CallFrame frame)
	{
		...
		if (frame.frameIndex > cx.getMaximumInterpreterStackDepth()) { throw Context
				.reportRuntimeError("Exceeded maximum stack depth"); }
		frame.frozen = false;
					...
					else
					{
						...
						if (scope == null || (frame.parentFrame != null && frame.parentFrame.scope == scope))
						{
							...
							break; // Never reached, but keeps the static analyzer
										// happy about "scope" not being null 5 lines
										// above.
						}
					}
				}
	...
	private static void dispatchEvents() throws Exception
	{
		if (Context.isDispatchingEventsEnabled && java.awt.EventQueue.isDispatchThread())
		{
			java.awt.EventQueue eventQ = java.awt.Toolkit.getDefaultToolkit().getSystemEventQueue();
			long time = System.currentTimeMillis() + 200; // max 0.2 seconds.
			while (System.currentTimeMillis() < time)
			{
				if (eventQ.peekEvent() == null)
				{
					if (lastEmptyTime < (System.currentTimeMillis() - 1000))
					{
						// Posting an extra events so that all events are flushed to
						// this quere (see EventQueue.getNextEvent() ->
						// SunToolkit.flushPendingEvents())
						eventQ.postEvent(new EmptyEvent());
					}
					else
					{
						break;
	...
	private static void do_add(Object[] stack, double[] sDbl, int stackTop, Context cx)
	{
		...
		else
		{
			...
			else if (lhs instanceof CharSequenceBuffer)
			{
				...
				{
					lhs = ((CharSequenceBuffer) lhs).append((CharSequenceBuffer) rhs);
				}
				else
				{
					lhs = ((CharSequenceBuffer) lhs).append(ScriptRuntime.toString(rhs));
				}
				stack[stackTop] = lhs;
			}
			else if (lhs instanceof String || rhs instanceof String)
			{
				if (rhs instanceof CharSequenceBuffer)
				{
					CharSequenceBuffer sb = new CharSequenceBuffer(ScriptRuntime.toString(lhs), (CharSequenceBuffer) rhs);
					stack[stackTop] = sb;
				}
				else
				{
					CharSequenceBuffer sb = new CharSequenceBuffer(ScriptRuntime.toString(lhs), ScriptRuntime
							.toString(rhs));
					stack[stackTop] = sb;
				}
			}
	...
	static long lastEmptyTime;

	private static class EmptyEvent extends AWTEvent implements ActiveEvent
	{
		public EmptyEvent()
		{
			super(new Object(), 0);
