...
public class Interpreter {
					...
					if (scope instanceof NativeCall) {
						...
						if (scope == null
							...
							break; // Never reached, but keeps the static
									// analyzer
									// happy about "scope" not being null 5
									// lines
									// above.
						}
					}
	...
	private static void dispatchEvents() throws Exception {
		if (Context.isDispatchingEventsEnabled
				&& java.awt.EventQueue.isDispatchThread()) {
			java.awt.EventQueue eventQ = java.awt.Toolkit.getDefaultToolkit()
					.getSystemEventQueue();
			...
			while (System.currentTimeMillis() < time) {
				if (eventQ.peekEvent() == null) {
					if (lastEmptyTime < (System.currentTimeMillis() - 1000)) {
						// Posting an extra events so that all events are
						// flushed to
