...
public class NativeGlobal implements Serializable, IdFunctionCall
{
	...
	public static void init(Context cx, Scriptable scope, boolean sealed)
	{
		...
		String[] errorMethods = Kit.semicolonSplit("" + "ConversionError;" + "EvalError;" + "RangeError;"
				+ "ReferenceError;" + "SyntaxError;" + "TypeError;" + "URIError;" + "InternalError;" + "JavaException;");
	 ...
	 * 'mask' argument supported by the C escape method, which used to be part of
	 * the browser imbedding. Blame for the strange constant names should be
	 * directed there.
	 */
	...
	private Object js_escape(Object[] args)
	{
		...
		{ // the 'mask' argument. Non-ECMA.
			...
			if (d != d || ((mask = (int) d) != d) || 0 != (mask & ~(URL_XALPHAS | URL_XPALPHAS | URL_PATH))) { throw Context
					.reportRuntimeError0("msg.bad.esc.mask"); }
		}
	...
	static boolean isEvalFunction(Object functionObj)
	{
		if (functionObj instanceof IdFunctionObject)
		{
			...
			if (function.hasTag(FTAG) && function.methodId() == Id_eval) { return true; }
		}
		return false;
	}
	...
	private static String encode(String str, boolean fullUri)
	{
		...
		for (int k = 0, length = str.length(); k != length; ++k)
		{
			...
			else
			{
				...
				if (0xDC00 <= C && C <= 0xDFFF) { throw Context.reportRuntimeError0("msg.bad.uri"); }
				int V;
				...
				{
					k++;
					if (k == length) { throw Context.reportRuntimeError0("msg.bad.uri"); }
					char C2 = str.charAt(k);
					if (!(0xDC00 <= C2 && C2 <= 0xDFFF)) { throw Context.reportRuntimeError0("msg.bad.uri"); }
