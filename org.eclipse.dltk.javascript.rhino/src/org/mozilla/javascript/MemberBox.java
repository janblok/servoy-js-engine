...
public final class MemberBox implements Serializable
{
	...
	Object invoke(Object target, Object[] args)
	{
		...
		try
		{
			...
			catch (IllegalAccessException ex)
			{
				...
				{
					if (!VMBridge.instance.tryToMakeAccessible(method)) { throw Context.throwAsScriptRuntimeEx(ex); }
				}
	...
	Object newInstance(Object[] args)
	{
		...
		try
		{
			...
			{
				if (!VMBridge.instance.tryToMakeAccessible(ctor)) { throw Context.throwAsScriptRuntimeEx(ex); }
			}
			return ctor.newInstance(args);
		}
		catch (Exception ex)
				...
				for (;;)
				{
					...
					if (Modifier.isPublic(c.getModifiers()))
					{
						try
						{
							...
							if (Modifier.isPublic(mModifiers) && !Modifier.isStatic(mModifiers)) { return m; }
						}
						catch (NoSuchMethodException ex)
						{
						}
	...
	/**
	 * Writes a Constructor or Method object. Methods and Constructors are not
	 * serializable, so we must serialize information about the class, the name,
	 * and the parameters and recreate upon deserialization.
	 */
	...
	/**
	 * Writes an array of parameter types to the stream. Requires special
	 * handling because primitive types cannot be found upon deserialization by
