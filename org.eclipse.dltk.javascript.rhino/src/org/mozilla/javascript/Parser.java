 ...
 * code. No tree rewriting is permitted at this stage, so that the parse tree is
 * a faithful representation of the source for frontend processing tools and
 * IDEs.
 * <p>
 * 
...
public class Parser {
	/**
	 * Maximum number of allowed function or constructor arguments, to follow
	 * SpiderMonkey.
	 */
	...
	// with token type
	final static int CLEAR_TI_MASK = 0xFFFF, // mask to clear token information
												// bits
	...
	void addWarning(String messageId, String messageArg, int position,
			int length) {
	...
	void reportError(String messageId, String messageArg) {
		if (ts == null) { // happens in some regression tests
			...
			reportError(messageId, messageArg, ts.tokenBeg, ts.tokenEnd
					- ts.tokenBeg);
		}
	}
	...
	private void recordComment(int lineno) {
		...
		Comment commentNode = new Comment(ts.tokenBeg, ts.getTokenLength(),
				ts.commentType, comment);
	...
	private boolean mustMatchToken(int toMatch, String messageId)
			throws IOException {
		return mustMatchToken(toMatch, messageId, ts.tokenBeg, ts.tokenEnd
				- ts.tokenBeg);
	}
	 ...
	 * 
	 * @return an {@link AstRoot} object representing the parsed program. If the
	 *         parse fails, {@code null} will be returned. (The parse failure
	 *         will result in a call to the {@link ErrorReporter} from
	 ...
	 * @throws IOException
	 *             if the {@link Reader} encounters an error
	 */
	...
	private FunctionNode function(int type) throws IOException {
		...
		try {
			...
			if (compilerEnv.isStrictMode()
				...
				String msg = (name != null && name.length() > 0) ? "msg.no.return.value"
		...
		if (memberExprNode != null) {
			...
			/*
			 * old code: if (memberExprNode != null) { pn =
			 * nf.createAssignment(Token.ASSIGN, memberExprNode, pn); if
			 * (functionType != FunctionNode.FUNCTION_EXPRESSION) { // XXX check
			 * JScript behavior: should it be createExprStatement? pn =
			 * nf.createExprStatementNoReturn(pn, baseLineno); } }
			 */
		}
	...
	private ConditionData condition() throws IOException {
		...
		if (data.condition instanceof Assignment) {
			addStrictWarning("msg.equal.as.assign", "",
					data.condition.getPosition(), data.condition.getLength());
		}
		return data;
	}
		...
		try {
			...
			if (pn != null) {
				if (compilerEnv.isStrictMode() && !pn.hasSideEffects()) {
					...
					addStrictWarning(
							pn instanceof EmptyExpression ? "msg.extra.trailing.semi"
									: "msg.no.side.effects", "", beg,
							nodeEnd(pn) - beg);
				}
				return pn;
			}
	 ...
	 * @param mask
	 *            mask for bits
	 * @return {@code true} if all the bits in the mask are set in "after" but
	 *         not in "before"
	 */
	...
	private AstNode returnOrYield(int tt, boolean exprContext)
		...
		if (insideFunction()
				&& nowAllSet(before, endFlags, Node.END_YIELDS
						| Node.END_RETURNS_VALUE)) {
		...
		if (labelSet == null) {
			...
			if (ls != null) {
				if (compilerEnv.isIdeMode()) {
					...
					reportError("msg.dup.label", dup.getAbsolutePosition(),
							dup.getLength());
				}
				reportError("msg.dup.label", label.getPosition(),
						label.getLength());
			}
		}
	...
	/**
	 * Found a name in a statement context. If it's a label, we gather up any
	 * following labels and the next non-label statement into a
	 * {@link LabeledStatement} "bundle" and return that. Otherwise we parse an
	 ...
	 * @param pos
	 *            the position where the node should start. It's sometimes the
	 *            var/const/let keyword, and other times the beginning of the
	 *            first token in the first variable declaration.
		...
		try {
			...
			if (isStatement && peekToken() == Token.LC) {
				...
				if (isStatement) {
					...
					ExpressionStatement es = new ExpressionStatement(pn,
							!insideFunction());
	...
	void defineSymbol(int declType, String name, boolean ignoreNotInBlock) {
		...
		Symbol symbol = definingScope != null ? definingScope.getSymbol(name)
				: null;
		int symDeclType = symbol != null ? symbol.getDeclType() : -1;
		if (symbol != null
				&& (symDeclType == Token.CONST || declType == Token.CONST || (definingScope == currentScope && symDeclType == Token.LET))) {
			addError(
					symDeclType == Token.CONST ? "msg.const.redecl"
	...
	private AstNode expr() throws IOException {
		...
		while (matchToken(Token.COMMA)) {
			...
			if (compilerEnv.isStrictMode() && !pn.hasSideEffects())
				addStrictWarning("msg.no.side.effects", "", pos, nodeEnd(pn)
						- pos);
	...
	private AstNode unaryExpr() throws IOException {
		...
		default:
			...
			consumeToken();
			UnaryExpression uexpr = new UnaryExpression(tt, ts.tokenBeg, pn,
					true);
	...
	private AstNode xmlInitializer() throws IOException {
		...
		for (;; tt = ts.getNextXMLToken()) {
			...
			case Token.XML:
				...
				int beg = ts.tokenBeg;
				AstNode expr = (peekToken() == Token.RC) ? new EmptyExpression(
						beg, ts.tokenEnd - beg) : expr();
	...
	/**
	 * Parse a new-expression, or if next token isn't {@link Token#NEW}, a
	 * primary expression.
	 * 
	 * @param allowCallSyntax
	 *            passed down to {@link #memberExprTail}
	 */
	...
	/**
	 * Parse any number of "(expr)", "[expr]" ".expr", "..expr", or ".(expr)"
	 * constructs trailing the passed expression.
	 * 
	 * @param pn
	 *            the non-null parent node
	 * @return the outermost (lexically last occurring) expression, which will
	 *         have the passed parent node as a descendant
	 */
	 ...
	 * 
	 * @param pn
	 *            the left-hand side (target) of the operator. Never null.
	 ...
	 * 
	 * @param atPos
	 *            a natural number if we just read an '@' token, else -1
	 * 
	 * @param s
	 *            the name or string that was matched (an identifier, "throw" or
	 *            "*").
	 * 
	 * @param memberTypeFlags
	 *            flags tracking whether we're a '.' or '..' child
	 * 
	...
	private AstNode destructuringPrimaryExpr() throws IOException,
			ParserException {
		try {
	...
	private AstNode arrayLiteral() throws IOException {
		...
		for (;;) {
			...
			if (tt == Token.COMMA) {
				...
				if (afterComma != -1)
					warnTrailingComma("msg.array.trailing.comma", pos,
							elements, afterComma);
				break;
	 ...
	 * 
	 * @param result
	 *            the first expression after the opening left-bracket
	 * @param pos
	 *            start of LB token that begins the array comprehension
	...
	private ObjectLiteral objectLiteral() throws IOException {
		...
		commaLoop: for (;;) {
			...
			case Token.STRING:
				...
				if ((tt == Token.NAME && peekToken() == Token.NAME && ("get"
						.equals(propertyName) || "set".equals(propertyName)))) {
					consumeToken();
					name = createNameNode();
					name.setJsDoc(jsdoc);
					ObjectProperty objectProp = getterSetterProperty(ppos,
							name, "get".equals(propertyName));
			...
			case Token.NUMBER:
				consumeToken();
				afterComma = -1;
				AstNode nl = new NumberLiteral(ts.tokenBeg, ts.getString(),
						ts.getNumber());
				nl.setJsDoc(jsdoc);
				propertyName = ts.getString();
			...
			case Token.RC:
				if (afterComma != -1 && compilerEnv.getWarnTrailingComma())
					warnTrailingComma("msg.extra.trailing.comma", pos, elems,
							afterComma);
				break commaLoop;
	...
	/**
	 * Create a {@code Name} node using the token info from the last scanned
	 * name. In some cases we need to either synthesize a name node, or we lost
	 * the name token information by peeking. If the {@code token} parameter is
	 * not {@link Token#NAME}, then we use token info saved in instance vars.
	 */
	...
	protected void checkActivationName(String name, int token) {
		...
		if ("arguments".equals(name)
				|| (compilerEnv.getActivationNames() != null && compilerEnv
						.getActivationNames().contains(name))) {
			activation = true;
	...
	private void checkCallRequiresActivation(AstNode pn) {
		if ((pn.getType() == Token.NAME && "eval".equals(((Name) pn)
				.getIdentifier()))
				|| (pn.getType() == Token.GETPROP && "eval"
						.equals(((PropertyGet) pn).getProperty()
								.getIdentifier())))
			setRequiresActivation();
	}
	 ...
	 * 
	 * @param pos
	 *            an offset into the input source stream. If the offset is
	 *            negative, it's converted to 0, and if it's beyond the end of
	 *            the source buffer, the last source position is used.
	 * 
	 * @return the offset of the beginning of the line containing pos (i.e. 1+
	 *         the offset of the first preceding newline). Returns -1 if the
	 *         {@link CompilerEnvirons} is not set to ide-mode, and
	 *         {@link #parse(java.io.Reader,String,int)} was used.
	 */
	...
	private void warnTrailingComma(String messageId, int pos, List<?> elems,
			int commaPos) {
	...
	/**
	 * Given a destructuring assignment with a left hand side parsed as an array
	 * or object literal and a right hand side expression, rewrite as a series
	 * of assignments to the variables defined in left from property accesses to
	 * the expression on the right.
	 * 
	 * @param type
	 *            declaration type: Token.VAR or Token.LET or -1
	 * @param left
	 *            array or object literal containing NAME nodes for variables to
	 *            assign
	 * @param right
	...
	Node destructuringAssignmentHelper(int variableType, Node left, Node right,
			String tempName) {
		Scope result = createScopeNode(Token.LETEXPR, left.getLineno());
		result.addChildToFront(new Node(Token.LET, createName(Token.NAME,
				tempName, right)));
		try {
			pushScope(result);
		...
		switch (left.getType()) {
		case Token.ARRAYLIT:
			empty = destructuringArray((ArrayLiteral) left, variableType,
					tempName, comma, destructuringNames);
			break;
		case Token.OBJECTLIT:
			empty = destructuringObject((ObjectLiteral) left, variableType,
					tempName, comma, destructuringNames);
			break;
		case Token.GETPROP:
		case Token.GETELEM:
	...
	boolean destructuringArray(ArrayLiteral array, int variableType,
			String tempName, Node parent, List<String> destructuringNames) {
		boolean empty = true;
		...
		for (AstNode n : array.getElements()) {
			...
			Node rightElem = new Node(Token.GETELEM, createName(tempName),
					createNumber(index));
			if (n.getType() == Token.NAME) {
				String name = n.getString();
				parent.addChildToBack(new Node(setOp, createName(
						Token.BINDNAME, name, null), rightElem));
				if (variableType != -1) {
	...
	boolean destructuringObject(ObjectLiteral node, int variableType,
			String tempName, Node parent, List<String> destructuringNames) {
		boolean empty = true;
		int setOp = variableType == Token.CONST ? Token.SETCONST
				: Token.SETNAME;
	 ...
	 * 
	 * @param token
	 *            the token of the node to create
	 * @param lineno
	...
	protected Node simpleAssignment(Node left, Node right) {
		...
		case Token.GETELEM: {
			...
			if (nodeType == Token.GETPROP) {
				type = Token.SETPROP;
				// TODO(stevey) - see
				// https://bugzilla.mozilla.org/show_bug.cgi?id=492036
				// The new AST code generates NAME tokens for GETPROP ids where
				// the old parser
				// generated STRING nodes. If we don't set the type to STRING
				// below, this will
	...
	private RuntimeException codeBug() throws RuntimeException {
		throw Kit.codeBug("ts.cursor=" + ts.cursor + ", ts.tokenBeg="
				+ ts.tokenBeg + ", currentToken=" + currentToken);
	}
}
