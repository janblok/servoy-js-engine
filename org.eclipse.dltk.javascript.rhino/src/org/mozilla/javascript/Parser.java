...
/**
 * This class implements the JavaScript parser. It is based on the C source
 * files jsparse.c and jsparse.h in the jsref package.
 * 
 * @see TokenStream
 * @author Mike McCabe
 * @author Brendan Eich
...
public class Parser {
	...
	/*
	 * Build a parse tree from the given sourceString. @return an Object
	 * representing the parsed program. If the parse fails, null will be
	 * returned. (The parse failure will result in a call to the ErrorReporter
	...
	private Node function(int functionType) throws IOException, ParserException {
		...
		fnNode.setSourceBounds(sourceStart, sourceEnd);
		...
		case Token.TRY: {
			...
			if (peek == Token.CATCH) {
				while (matchToken(Token.CATCH)) {
					...
					nf.addChildToBack(
							catchblocks,
							nf.createCatch(varName, catchCond, statements(),
									ts.getLineno()));
