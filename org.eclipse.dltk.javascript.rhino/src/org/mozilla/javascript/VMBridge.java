...
public abstract class VMBridge {
	...
	private static VMBridge makeInstance() {
		String[] classNames = { "org.mozilla.javascript.VMBridge_custom",
	 ...
	 * <p>
	 * The runtime will pass the resulting helper object to the subsequent calls
	 * to {@link #getContext(Object contextHelper)} and
	 * {@link #setContext(Object contextHelper, Context cx)} methods. In this
	 * way the implementation can use the helper to cache information about
	 * current thread to make {@link Context} access faster.
	 */
	protected abstract Object getThreadContextHelper();

	/**
	 * Get {@link Context} instance associated with the current thread or null
	 * if none.
	 * 
	 * @param contextHelper
	 *            The result of {@link #getThreadContextHelper()} called from
	 *            the current thread.
	 */
	 ...
	 * default (Sun Bug #4071593). VMBridge instance should try to workaround
	 * that via, for example, calling method.setAccessible(true) when it is
	 * available. The implementation is responsible to catch all possible
	 * exceptions like SecurityException if the workaround is not available.
	 * 
	 * @return true if it was possible to make method accessible or false
	 *         otherwise.
	 */
	...
	/**
	 * Returns whether or not a given member (method or constructor) has
	 * variable arguments. Variable argument methods have only been supported in
	 * Java since JDK 1.5.
	 */
