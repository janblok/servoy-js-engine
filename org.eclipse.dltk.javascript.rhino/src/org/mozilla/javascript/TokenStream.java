...
class TokenStream
{
	...
	/*
	 * This function uses the cached op, string and number fields in TokenStream;
	 * if getToken has been called since the passed token was scanned, the op or
	 * string printed may be incorrect.
	 */
	...
	void readRegExp(int startToken) throws IOException
	{
		...
		if (isAlpha(peekChar())) { throw parser.reportError("msg.invalid.re.flag"); }
	...
	{
		if (ungetCursor != 0) { return ungetBuffer[--ungetCursor]; }
