 ...
 * @author Mike McCabe
 */
public class NativeArray extends IdScriptableObject implements Wrapper
{
	 ...
	 * Most array operations go through getElem or setElem (defined in this file)
	 * to handle the full 2^32 range; it might be faster to have versions of most
	 * of the loops in this file for the (infinitely more common) case of indices <
	 * 2^31. - Functions that need a new Array call "new Array" in the current
	 * scope rather than using a hardwired constructor; "Array" could be
	 * redefined. It turns out that js calls the equivalent of "new Array" in the
	 * current scope, except that it always gets at least an object back, even
	 * when Array == null.
	 */
	...
	{
		NativeArray prototype = new NativeArray();
	...
	{
		if (s.equals("length")) { return instanceIdInfo(DONTENUM | PERMANENT, Id_length); }
	...
	{
		if (id == Id_length) { return ScriptRuntime.wrapNumber(length); }
	...
	{
		if (!f.hasTag(ARRAY_TAG)) { return super.execIdCall(f, cx, scope, thisObj, args); }
		int id = f.methodId();
		switch (id)
		{
			case Id_constructor:
	...
	private static long toArrayIndex(String id)
	{
		...
		if (d == d)
		{
			...
			if (index == d && index != 4294967295L)
			{
				...
				if (Long.toString(index).equals(id)) { return index; }
			}
		}
		return -1;
	}
	...
	{
		/*
		 * XXX do we satisfy this? 15.4.5.1 [[Put]](P, V): 1. Call the [[CanPut]]
		 * method of A with name P. 2. If Result(1) is false, return. ?
		 */
	 ...
	 * be generic; anything that has a length property is assumed to be an array.
	 * getLengthProperty returns 0 if obj does not have the length property or
	 * its value is not convertible to a number.
	 */
	static long getLengthProperty(Context cx, Scriptable obj)
	{
		...
		else if (obj instanceof NativeArray) { return ((NativeArray) obj).getLength(); }
			...
			boolean toLocale)
	{
		 ...
		 * StringBuffers are limited to 2^31 in java.
		 */
			...
			if (!iterating)
			{
				...
				for (int j = 0; j < ids.length; j++)
				{
					if (ids[j] instanceof String)
					{
						...
						result.append(ids[j]);
						result.append("=");
						result.append(ScriptRuntime.toString(thisObj.get((String) ids[j], thisObj)));
					}
				}
			}
		}
	...
	private static String js_join(Context cx, Scriptable thisObj, Object[] args)
	{
		...
		if (llength != length) { throw Context.reportRuntimeError1("msg.arraylength.too.big", String.valueOf(llength)); }
	...
	private static boolean isBigger(Context cx, Scriptable scope, Object x, Object y, Object cmp, Object[] cmpBuf)
	{
		...
		else if (undef == x) { return true; // y != undef here, so x > y
		}
	...
	private static Object js_shift(Context cx, Scriptable thisObj, Object[] args)
	{
		...
		if (length > 0)
		{
			...
			/*
			 * Slide down the array above the first element. Leave i set to point
			 * to the last element.
			 */
			if (length > 0)
			{
	...
	private static Object js_splice(Context cx, Scriptable scope, Scriptable thisObj, Object[] args)
	{
		...
		if (count != 0)
		{
			if (count == 1 && (cx.getLanguageVersion() == Context.VERSION_1_2))
			{
				 ...
				 * argument, for instance. JS1.2 emulated Perl too closely and
				 * returned a non-Array for the single-splice-out case, requiring
				 * callers to test and wrap in [] if necessary. So JS1.3, default,
				 * and other versions all return an array of length 1 for
				 * uniformity.
				 */
	...
	private static Scriptable js_concat(Context cx, Scriptable scope, Scriptable thisObj, Object[] args)
	{
		...
		/*
		 * Put the target in the result array; only add it as an array if it looks
		 * like one.
		 */
		...
		/*
		 * Copy from the arguments into the result. If any argument has a numeric
		 * length property, treat it as an array and add elements separately;
	...
	private Object indexOfHelper(Context cx, Scriptable thisObj, Object[] args, boolean isLast)
	{
		...
		if (isLast)
		{
			...
			{
				if (ScriptRuntime.shallowEq(getElem(cx, thisObj, i), compareTo)) { return new Long(i); }
			}
		}
		else
		{
	...
	private Object iterativeMethod(Context cx, int id, Scriptable scope, Scriptable thisObj, Object[] args)
	{
		...
		if (callbackArg == null || !(callbackArg instanceof Function)) { throw ScriptRuntime
				.notFunctionError(ScriptRuntime.toString(callbackArg)); }
