...
public class NodeTransformer {
	...
	private void transformCompilationUnit_r(final ScriptNode tree,
			final Node parent, Scope scope, boolean createScopeObjects,
		...
		siblingLoop: for (;;) {
			...
			if (createScopeObjects
				...
				if (newScope.getSymbolTable() != null) {
					...
					for (String name : newScope.getSymbolTable().keySet()) {
						innerLet.addChildToBack(Node
								.newString(Token.NAME, name));
					}
					newScope.setSymbolTable(null); // so we don't transform
													// again
					Node oldNode = node;
			...
			case Token.RETURN: {
				...
				/*
				 * If we didn't support try/finally, it wouldn't be necessary to
				 * put LEAVEWITH nodes here... but as we do need a series of JSR
				 * FINALLY nodes before each RETURN, we need to ensure that each
				 * finally block gets the correct scope... which could mean that
			...
			case Token.VAR: {
				...
				for (Node cursor = node.getFirstChild(); cursor != null;) {
					...
					if (n.getType() == Token.NAME) {
						...
						n = new Node(type == Token.CONST ? Token.SETCONST
								: Token.SETNAME, n, init);
					} else {
			...
			case Token.IFNE: {
				/*
				 * We want to suppress warnings for undefined property o.p for
				 * the following constructs: typeof o.p, if (o.p), if (!o.p), if
				 * (o.p == undefined), if (undefined == o.p)
				 */
				Node child = node.getFirstChild();
				if (type == Token.IFNE) {
			...
			for (Node v = vars.getFirstChild(); v != null; v = v.getNext()) {
				...
				if (current.getType() == Token.LETEXPR) {
					...
					if (isExpression) {
						...
						body = new Node(Token.BLOCK, new Node(Token.EXPR_VOID,
								c.getNext()), body);
					}
					...
					if (destructuringNames != null) {
						...
						for (int i = 0; i < destructuringNames.size(); i++) {
							objectLiteral.addChildToBack(new Node(Token.VOID,
									Node.newNumber(0.0)));
						}
					}
