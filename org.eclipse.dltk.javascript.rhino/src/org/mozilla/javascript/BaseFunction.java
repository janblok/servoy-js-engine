...
public class BaseFunction extends IdScriptableObject implements Function
{
	...
	public boolean hasInstance(Scriptable instance)
	{
		...
		if (protoProp instanceof Scriptable) { return ScriptRuntime.jsDelegatesTo(instance, (Scriptable) protoProp); }
	...
	{
		if (!f.hasTag(FUNCTION_TAG)) { return super.execIdCall(f, cx, scope, thisObj, args); }
		int id = f.methodId();
		switch (id)
		{
			case Id_constructor:
	...
	private BaseFunction realFunction(Scriptable thisObj, IdFunctionObject f)
	{
		...
		if (x instanceof BaseFunction) { return (BaseFunction) x; }
	...
	{
		if ((prototypePropertyAttributes & READONLY) != 0) { throw new IllegalStateException(); }
		prototypeProperty = (value != null) ? value : UniqueTag.NULL_VALUE;
	...
	protected Scriptable getClassPrototype()
	{
		...
		if (protoVal instanceof Scriptable) { return (Scriptable) protoVal; }
	...
	public Scriptable construct(Context cx, Scriptable scope, Object[] args)
	{
		...
		else
		{
			...
			if (!(val instanceof Scriptable))
			{
				...
				throw new IllegalStateException("Bad implementaion of call as constructor, name=" + getFunctionName()
						+ " in " + getClass().getName());
			}
			result = (Scriptable) val;
	 ...
	 * {@link #construct} uses the method to to get the value for
	 * <tt>thisObj</tt> argument when invoking {@link #call}. The methos is
	 * allowed to return <tt>null</tt> to indicate that {@link #call} will
	 * create a new object itself. In this case {@link #construct} will set scope
	 * and prototype on the result {@link #call} unless they are already set.
	 */
	...
	private static Object jsConstructor(Context cx, Scriptable scope, Object[] args)
	{
		...
		/*
		 * version != 1.2 Function constructor behavior - print 'anonymous' as the
		 * function name if the version (under which the function was compiled) is
		 * less than 1.2... or if it's greater than 1.2, because we need to be
		 * closer to ECMA.
		 */
		...
		// mode.
		return cx.compileFunction(global, source, new Interpreter(), reporter, sourceURI, 1, null);
	}
	...
	private static final int Id_constructor = 1, Id_toString = 2, Id_toSource = 3, Id_apply = 4, Id_call = 5,
