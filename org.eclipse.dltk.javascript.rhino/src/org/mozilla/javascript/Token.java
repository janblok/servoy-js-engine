...
public class Token
{
	...
	// start enum
			...
			ENTERWITH = 2, LEAVEWITH = 3, RETURN = 4, GOTO = 5, IFEQ = 6, IFNE = 7, SETNAME = 8,
			BITOR = 9,
			BITXOR = 10,
			BITAND = 11, EQ = 12, NE = 13, LT = 14, LE = 15, GT = 16, GE = 17, LSH = 18, RSH = 19,
			URSH = 20,
			ADD = 21,
			SUB = 22, MUL = 23, DIV = 24, MOD = 25, NOT = 26, BITNOT = 27, POS = 28, NEG = 29,
			NEW = 30,
			DELPROP = 31,
			TYPEOF = 32, GETPROP = 33, SETPROP = 34, GETELEM = 35, SETELEM = 36, CALL = 37,
			NAME = 38,
			NUMBER = 39,
			STRING = 40, NULL = 41, THIS = 42, FALSE = 43, TRUE = 44,
			SHEQ = 45, // shallow equality (===)
			SHNE = 46, // shallow inequality (!==)
			REGEXP = 47, BINDNAME = 48, THROW = 49,
			RETHROW = 50, // rethrow caught execetion: catch (e if ) use it
			IN = 51, INSTANCEOF = 52, LOCAL_LOAD = 53, GETVAR = 54, SETVAR = 55,
			CATCH_SCOPE = 56,
			ENUM_INIT_KEYS = 57,
			ENUM_INIT_VALUES = 58, ENUM_NEXT = 59, ENUM_ID = 60, THISFN = 61, RETURN_RESULT = 62, // to
																																// return
																																// prevoisly
																																// stored
																																// return
			...
			ESCXMLATTR = 71, ESCXMLTEXT = 72, REF_MEMBER = 73, // Reference for
																				// x.@y, x..y etc.
	...
	HOOK = 98, // conditional (?:)
			...
			LABEL = 126, // label
			TARGET = 127, LOOP = 128, EXPR_VOID = 129, // expression statement in
																		// functions
			...
			TO_OBJECT = 145, TO_DOUBLE = 146,

			GET = 147, // JS 1.5 get pseudo keyword
			SET = 148, // JS 1.5 set pseudo keyword
			CONST = 149, SETCONST = 150, SETCONSTVAR = 151, LAST_TOKEN = 152;

	public static String name(int token)
	{
		if (!printNames) { return String.valueOf(token); }
		switch (token)
		{
			case ERROR:
				return "ERROR";
