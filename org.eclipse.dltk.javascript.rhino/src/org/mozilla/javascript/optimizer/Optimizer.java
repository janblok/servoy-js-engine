...
class Optimizer
{
	...
	private void optimizeFunction(OptFunctionNode theFunction)
	{
		...
		if (!theFunction.fnode.requiresActivation())
		{
			 ...
			 * assignment op involving just Number typed vars is marked so that the
			 * codegen will generate non-object code.
			 */
	...
	private int rewriteForNumberVariables(Node n)
	{
		switch (n.getType())
		{
			...
			case Token.DEC:
			{
				...
				else if (child.getType() == Token.GETELEM) { return rewriteForNumberVariables(child); }
				return NoType;
			}
			case Token.SETVAR:
			{
