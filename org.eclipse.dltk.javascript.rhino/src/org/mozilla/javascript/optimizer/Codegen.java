...
public class Codegen implements Evaluator {
	...
	private RuntimeException reportClassFileFormatException(
		...
		String msg = scriptOrFn instanceof FunctionNode ? ScriptRuntime
				.getMessage2("msg.while.compiling.fn",
	...
	private byte[] generateCode(String encodedSource) {
		...
		if (directCallTargets != null) {
			...
			for (int j = 0; j != N; ++j) {
				cfw.addField(getDirectTargetFieldName(j), mainClassSignature,
						ClassFileWriter.ACC_PRIVATE);
			}
		}
	...
	private void emitDirectConstructor(ClassFileWriter cfw, OptFunctionNode ofn) {
		...
		cfw.addInvoke(ByteCode.INVOKESTATIC, mainClassName,
				getBodyMethodName(ofn.fnode), getBodyMethodSignature(ofn.fnode));
	...
	private void generateFunctionInit(ClassFileWriter cfw, OptFunctionNode ofn) {
		...
		if (regexpCount != 0) {
			...
			cfw.add(ByteCode.PUTFIELD, mainClassName, REGEXP_ARRAY_FIELD_NAME,
	...
	void pushRegExpArray(ClassFileWriter cfw, ScriptNode n, int contextArg,
			int scopeArg) {
		...
		for (int i = 0; i != regexpCount; ++i) {
			...
			cfw.addInvoke(ByteCode.INVOKEINTERFACE,
					"org/mozilla/javascript/RegExpProxy", "wrapRegExp",
					"(Lorg/mozilla/javascript/Context;"
							+ "Lorg/mozilla/javascript/Scriptable;"
							+ "Ljava/lang/Object;"
							+ ")Lorg/mozilla/javascript/Scriptable;");
...
class BodyCodegen {
	...
	private void generateGenerator() {
		...
		if (fnCurrent != null
				...
				&& (!compilerEnv.isUseDynamicScope() || fnCurrent.fnode
		...
		cfw.add(ByteCode.DUP);
		if (isTopLevel)
			Kit.codeBug(); // Only functions can be generators
		cfw.add(ByteCode.ALOAD_0);
		cfw.add(ByteCode.GETFIELD, codegen.mainClassName,
				Codegen.DIRECT_CALL_PARENT_FIELD, codegen.mainClassSignature);
		cfw.add(ByteCode.PUTFIELD, codegen.mainClassName,
				Codegen.DIRECT_CALL_PARENT_FIELD, codegen.mainClassSignature);
	...
	private void generatePrologue() {
		...
		if (isGenerator) {
			...
			if (targets != null) {
				...
				generatorSwitch = cfw.addTableSwitch(0, targets.size()
						+ GENERATOR_START);
		...
		if (hasVarsInRegs) {
			...
			if (parmCount > 0 && !inDirectCallFunction) {
				...
				cfw.addALoad(argsLocal);
				cfw.addPush(parmCount);
				addScriptRuntimeInvoke("padArguments", "([Ljava/lang/Object;I"
						+ ")[Ljava/lang/Object;");
				...
				for (Node n : finallys.keySet()) {
					if (n.getType() == Token.FINALLY) {
						...
						for (int i = 0; i < ret.jsrPoints.size(); i++) {
							...
							cfw.add(ByteCode.GOTO, ret.jsrPoints.get(i)
									.intValue());
							c++;
						}
					}
	...
	private void generateStatement(Node node) {
		...
		case Token.ENUM_INIT_ARRAY:
			generateExpression(child, node);
			cfw.addALoad(contextLocal);
			int enumType = type == Token.ENUM_INIT_KEYS ? ScriptRuntime.ENUMERATE_KEYS
					: type == Token.ENUM_INIT_VALUES ? ScriptRuntime.ENUMERATE_VALUES
	...
	private void generateExpression(Node node, Node parent) {
		...
		case Token.ADD: {
			...
			default:
				if (child.getType() == Token.STRING) {
					addScriptRuntimeInvoke("add", "(Ljava/lang/String;"
		...
		case Token.DIV:
		case Token.MOD:
			visitArithmetic(node, type == Token.DIV ? ByteCode.DDIV
		...
		case Token.GETELEM:
			...
			cfw.addALoad(contextLocal);
			if (node.getIntProp(Node.ISNUMBER_PROP, -1) != -1) {
				addScriptRuntimeInvoke("getObjectIndex", "(Ljava/lang/Object;D"
						+ "Lorg/mozilla/javascript/Context;"
						+ ")Ljava/lang/Object;");
			} else {
				cfw.addALoad(variableObjectLocal);
	...
	private void visitOptimizedCall(Node node, OptFunctionNode target,
		...
		cfw.addInvoke(ByteCode.INVOKESTATIC, codegen.mainClassName,
				(type == Token.NEW) ? codegen.getDirectCtorName(target.fnode)
						: codegen.getBodyMethodName(target.fnode), codegen
		...
		for (int i = 0; i != argCount; ++i) {
			...
			if (!directCall) {
				...
				if (dcp_register >= 0) {
					...
					generateExpression(argChild, node);
					int childNumberFlag = argChild.getIntProp(
							Node.ISNUMBER_PROP, -1);
	...
	private void visitTryCatchFinally(Jump node, Node child) {
		...
		if (finallyTarget != null) {
			...
			// mark the handler
			cfw.addExceptionHandler(startLabel, finallyLabel, finallyHandler,
					null); // catch any
		}
	...
	private void visitIncDec(Node node) {
		...
		case Token.GETELEM: {
			...
			if (elemChild.getNext().getIntProp(Node.ISNUMBER_PROP, -1) != -1) {
				addOptRuntimeInvoke("elemIncrDecr", "(Ljava/lang/Object;" + "D"
						+ "Lorg/mozilla/javascript/Context;" + "I"
						+ ")Ljava/lang/Object;");
			} else {
	...
	private static boolean isArithmeticNode(Node node) {
		int type = node.getType();
		return (type == Token.SUB) || (type == Token.MOD)
				|| (type == Token.DIV) || (type == Token.MUL);
	}
	...
	private void visitIfJumpRelOp(Node node, Node child, int trueGOTO,
		...
		if (type == Token.INSTANCEOF || type == Token.IN) {
			...
			cfw.addALoad(contextLocal);
			addScriptRuntimeInvoke((type == Token.INSTANCEOF) ? "instanceOf"
					: "in", "(Ljava/lang/Object;" + "Ljava/lang/Object;"
					+ "Lorg/mozilla/javascript/Context;" + ")Z");
		...
		if (childNumberFlag != -1) {
			...
			String routine = ((type == Token.LT) || (type == Token.GT)) ? "cmp_LT"
					: "cmp_LE";
	...
	private void visitGetProp(Node node, Node child) {
		...
		if (node.getType() == Token.GETPROPNOWARN) {
			cfw.addALoad(contextLocal);
			addScriptRuntimeInvoke("getObjectPropNoWarn", "(Ljava/lang/Object;"
					+ "Ljava/lang/String;" + "Lorg/mozilla/javascript/Context;"
					+ ")Ljava/lang/Object;");
			return;
		}
	...
	private void visitSetElem(int type, Node node, Node child) {
		...
		if (type == Token.SETELEM_OP) {
			if (indexIsNumber) {
				...
				cfw.addALoad(contextLocal);
				addOptRuntimeInvoke("getObjectIndex", "(Ljava/lang/Object;D"
						+ "Lorg/mozilla/javascript/Context;"
						+ ")Ljava/lang/Object;");
			} else {
	...
	private void visitDotQuery(Node node, Node child) {
		...
		addScriptRuntimeInvoke("toBoolean", "(Ljava/lang/Object;)Z");
		cfw.addALoad(variableObjectLocal);
		addScriptRuntimeInvoke("updateDotQuery", "(Z"
				+ "Lorg/mozilla/javascript/Scriptable;" + ")Ljava/lang/Object;");
		cfw.add(ByteCode.DUP);
