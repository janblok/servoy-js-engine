...
public class ScriptRuntime
{
	 ...
	 * the compiler sticks in a test of a static field in the enclosing class for
	 * null and the code for creating the class value. It has to do this since
	 * the reference has to get pushed off til executiontime (i.e. can't force an
	 * early load), but for the 'standard' classes - especially those in
	 * java.lang, we can trust that they won't cause problems by being loaded
	 * early.
	 */

	public final static Class BooleanClass = Kit.classOrNull("java.lang.Boolean"), ByteClass = Kit
			.classOrNull("java.lang.Byte"), CharacterClass = Kit.classOrNull("java.lang.Character"), ClassClass = Kit
			.classOrNull("java.lang.Class"), DoubleClass = Kit.classOrNull("java.lang.Double"), FloatClass = Kit
			.classOrNull("java.lang.Float"), IntegerClass = Kit.classOrNull("java.lang.Integer"), LongClass = Kit
			.classOrNull("java.lang.Long"), NumberClass = Kit.classOrNull("java.lang.Number"), ObjectClass = Kit
			.classOrNull("java.lang.Object"), ShortClass = Kit.classOrNull("java.lang.Short"), StringClass = Kit
			.classOrNull("java.lang.String"), DateClass = Kit.classOrNull("java.util.Date");

	public final static Class ContextClass = Kit.classOrNull("org.mozilla.javascript.Context"),
			ContextFactoryClass = Kit.classOrNull("org.mozilla.javascript.ContextFactory"), FunctionClass = Kit
					.classOrNull("org.mozilla.javascript.Function"), ScriptableClass = Kit
					.classOrNull("org.mozilla.javascript.Scriptable"), ScriptableObjectClass = Kit
					.classOrNull("org.mozilla.javascript.ScriptableObject");

	private static final String[] lazilyNames = { "RegExp", "org.mozilla.javascript.regexp.NativeRegExp", "Packages",
			"org.mozilla.javascript.NativeJavaTopPackage", "java", "org.mozilla.javascript.NativeJavaTopPackage",
			"getClass", "org.mozilla.javascript.NativeJavaTopPackage", "JavaAdapter",
			"org.mozilla.javascript.JavaAdapter", "JavaImporter", "org.mozilla.javascript.ImporterTopLevel",
			// hack using literal string (xml) just to minimize changes for now
	...
	public static boolean isRhinoRuntimeType(Class cl)
	{
		...
		{
			return (cl == StringClass || cl == BooleanClass || NumberClass.isAssignableFrom(cl) || ScriptableClass
					.isAssignableFrom(cl));
		}
	}
	...
	static double stringToNumber(String s, int start, int radix)
	{
		...
		if (sum >= 9007199254740992.0)
		{
			...
			{
				/*
				 * If we're accumulating a decimal number and the number is >= 2^53,
				 * then the result from the repeated multiply-add above may be
				 * inaccurate. Call Java to get the correct answer.
				 */
				try
				{
	...
	public static String toString(Object val)
	{
		...
		{
			if (val == null) { return ""; }
			if (val == Undefined.instance) { return "undefined"; }
	...
	public static String numberToString(double d, int base)
	{
		...
		if ((base < 2) || (base > 36)) { throw Context.reportRuntimeError1("msg.bad.radix", Integer.toString(base)); }
	...
	static String uneval(Context cx, Scriptable scope, Object value)
	{
		...
		if (value instanceof Number)
		{
			...
			if (d == 0 && 1 / d < 0) { return "-0"; }
			return toString(d);
		}
		if (value instanceof Boolean) { return toString(value); }
	...
	{
		if (val instanceof Scriptable) { return (Scriptable) val; }
	...
	public static Scriptable toObjectOrNull(Context cx, Object obj)
	{
		...
		else if (obj != null && obj != Undefined.instance) { return toObject(cx, getTopCallScope(cx), obj); }
		return null;
	}
	...
	{
		if (val instanceof Scriptable) { return (Scriptable) val; }
		if (val == null) { throw typeError0("msg.null.to.object"); }
		if (val == Undefined.instance) { throw typeError0("msg.undef.to.object"); }
		String className = val instanceof String ? "String" : val instanceof CharSequenceBuffer ? "String"
				: val instanceof Number ? "Number" : val instanceof Boolean ? "Boolean" : val instanceof Date ? "Date"
						: null;
		if (className != null)
		{
			Object[] args = { val };
	...
	{
		if (!(fun instanceof Function)) { throw notFunctionError(toString(fun)); }
		Function function = (Function) fun;
		Scriptable thisObj = toObjectOrNull(cx, thisArg);
		if (thisObj == null) { throw undefCallError(thisObj, "function"); }
	...
	public static Object setDefaultNamespace(Object namespace, Context cx)
	{
		...
		if (!scope.has(DEFAULT_NS_TAG, scope))
		{
			...
			ScriptableObject.defineProperty(scope, DEFAULT_NS_TAG, ns, ScriptableObject.PERMANENT
	...
	public static Object searchDefaultNamespace(Context cx)
	{
		...
		for (;;)
		{
			...
			if (parent == null)
			{
				...
				if (nsObject == Scriptable.NOT_FOUND) { return null; }
				break;
			}
	...
	static Function getExistingCtor(Context cx, Scriptable scope, String constructorName)
	{
		...
		if (ctorVal instanceof Function) { return (Function) ctorVal; }
			...
			if (0 <= c && c <= 9 && len <= (negate ? MAX_VALUE_LENGTH + 1 : MAX_VALUE_LENGTH))
			{
				...
				if (i == len
						&& (oldIndex > (Integer.MIN_VALUE / 10) || (oldIndex == (Integer.MIN_VALUE / 10) && c <= (negate ? -(Integer.MIN_VALUE % 10)
								: (Integer.MAX_VALUE % 10))))) { return 0xFFFFFFFFL & (negate ? index : -index); }
			}
		}
		return -1L;
	}
	...
	static Object getIndexObject(String s)
	{
		...
		if (indexTest >= 0) { return new Integer((int) indexTest); }
		return s;
	}
	...
	public static Object getObjectElem(Object obj, Object elem, Context cx)
	{
		...
		if (sobj == null) { throw undefReadError(obj, elem); }
	...
	{
		Scriptable sobj = toObjectOrNull(cx, obj);
		if (sobj == null) { throw undefReadError(obj, property); }
	...
	{
		Scriptable sobj = toObjectOrNull(cx, obj);
		if (sobj == null) { throw undefReadError(obj, toString(dblIndex)); }
	...
	{
		Scriptable sobj = toObjectOrNull(cx, obj);
		if (sobj == null) { throw undefWriteError(obj, elem, value); }
	...
	{
		Scriptable sobj = toObjectOrNull(cx, obj);
		if (sobj == null) { throw undefWriteError(obj, property, value); }
	...
	{
		Scriptable sobj = toObjectOrNull(cx, obj);
		if (sobj == null) { throw undefWriteError(obj, String.valueOf(dblIndex), value); }
	...
	/**
	 * The delete operator See ECMA 11.4.1 In ECMA 0.19, the description of the
	 * delete operator (11.4.1) assumes that the [[Delete]] method returns a
	 * value. However, the definition of the [[Delete]] operator (8.6.2.5) does
	 * not define a return value. Here we assume that the [[Delete]] method
			...
			boolean asFunctionCall)
	{
		...
		for (;;)
		{
			...
			if (parentScope == null)
			{
				...
				if (result == Scriptable.NOT_FOUND)
				{
					if (firstXMLObject == null || asFunctionCall) { throw notFoundError(scope, name); }
		...
		if (asFunctionCall)
		{
			if (!(result instanceof Callable)) { throw notFunctionError(result, name); }
	...
	/**
	 * Returns the object in the scope chain that has a given property. The order
	 * of evaluation of an assignment expression involves evaluating the lhs to a
	 * reference, evaluating the rhs, and then modifying the reference with the
	 * rhs value. This method is used to 'bind' the given name to an object
		...
		childScopesChecks: if (parent != null)
		{
			...
			while (scope instanceof NativeWith)
			{
				...
				if (withObj instanceof XMLObject)
				{
					...
					if (xmlObject.ecmaHas(cx, id)) { return xmlObject; }
					if (firstXMLObject == null)
					{
			...
			for (;;)
			{
				if (ScriptableObject.hasProperty(scope, id)) { return scope; }
				scope = parent;
				parent = parent.getParentScope();
				if (parent == null)
				{
	 ...
	 * enumerated only once across multiple objects in a prototype chain. XXX -
	 * ECMA delete doesn't hide properties in the prototype, but js/ref does.
	...
	{
		String s = toStringIdOrIndex(cx, elem);
		if (s != null) { return getPropFunctionAndThis(obj, s, cx); }
		int index = lastIndexResult(cx);

		Scriptable thisObj = toObjectOrNull(cx, obj);
		if (thisObj == null) { throw undefCallError(obj, String.valueOf(index)); }
		...
		if (!(value instanceof Callable)) { throw notFunctionError(value, elem); }
	...
	{
		Scriptable thisObj = toObjectOrNull(cx, obj);
		if (thisObj == null) { throw undefCallError(obj, property); }
	...
	{
		if (!(value instanceof Callable)) { throw notFunctionError(value); }
	...
	public static Ref callRef(Callable function, Scriptable thisObj, Object[] args, Context cx)
	{
		if (function instanceof RefCallable)
		{
			...
			if (ref == null) { throw new IllegalStateException(rfunction.getClass().getName() + ".refCall() returned null"); }
			return ref;
		}
	...
	{
		if (!(fun instanceof Function)) { throw notFunctionError(fun); }
		Function function = (Function) fun;
		...
		{
			if ("Date".equals(((BaseFunction) function).getFunctionName()) && args != null && args.length == 1
			...
			Scriptable callerThis, int callType, String filename, int lineNumber)
	{
		...
		{
			if (NativeGlobal.isEvalFunction(fun)) { return evalSpecial(cx, scope, callerThis, args, filename, lineNumber); }
		}
		else if (callType == Node.SPECIALCALL_WITH)
		{
			if (NativeWith.isWithFunction(fun)) { throw Context.reportRuntimeError1("msg.only.from.new", "With"); }
		}
		else
		{
			throw Kit.codeBug();
	...
	public static Object newSpecial(Context cx, Object fun, Object[] args, Scriptable scope, int callType)
	{
		if (callType == Node.SPECIALCALL_EVAL)
		{
			if (NativeGlobal.isEvalFunction(fun)) { throw typeError1("msg.not.ctor", "eval"); }
		}
		else if (callType == Node.SPECIALCALL_WITH)
		{
			if (NativeWith.isWithFunction(fun)) { return NativeWith.newWithSpecial(cx, scope, args); }
		}
		else
		{
			throw Kit.codeBug();
	...
	public static Object applyOrCall(boolean isApply, Context cx, Scriptable scope, Scriptable thisObj, Object[] args)
	{
		...
		else
		{
			...
			if (!(value instanceof Callable)) { throw ScriptRuntime.notFunctionError(value, thisObj); }
			...
			int lineNumber)
	{
		...
		{
			if (cx.hasFeature(Context.FEATURE_STRICT_MODE) || cx.hasFeature(Context.FEATURE_STRICT_EVAL)) { throw Context
	...
	public static Object add(Object val1, Object val2, Context cx)
	{
		if (val1 == null && val2 == null)
			return null;
		if (val1 instanceof Number && val2 instanceof Number) { return wrapNumber(((Number) val1).doubleValue()
				+ ((Number) val2).doubleValue()); }
		if (val1 instanceof XMLObject)
		{
			Object test = ((XMLObject) val1).addValues(cx, true, val2);
			if (test != Scriptable.NOT_FOUND) { return test; }
		}
		...
		if (val1 instanceof CharSequenceBuffer)
		{
			...
			{
				return ((CharSequenceBuffer) val1).append((CharSequenceBuffer) val2);
			}
			else
			{
	...
	public static Object propIncrDecr(Object obj, String id, Context cx, int incrDecrMask)
	{
		...
		if (start == null) { throw undefReadError(obj, id); }
	...
	{
		if (!(val instanceof Scriptable)) { return val; }
	...
	public static boolean eq(Object x, Object y)
	{
		...
		else if (y instanceof CharSequenceBuffer) { return y.equals(x); }
		...
		if (x instanceof Date)
		{
			...
			if (y instanceof Date) { return ((Date) x).getTime() == ((Date) y).getTime(); }
			return false;
		}
		else if (y instanceof Date && x instanceof Wrapper)
		{
			x = ((Wrapper)x).unwrap();
			if (x instanceof Date) { return ((Date) x).getTime() == ((Date) y).getTime(); }
			return false;
		}

		if (x == null || x == Undefined.instance)
		{
			if (y == null || y == Undefined.instance) { return true; }
			if (y instanceof ScriptableObject)
			{
				Object test = ((ScriptableObject) y).equivalentValues(x);
				if (test != Scriptable.NOT_FOUND) { return ((Boolean) test).booleanValue(); }
			}
			return false;
		}
		...
		else if (x instanceof Boolean)
		{
			...
			if (y instanceof Boolean) { return b == ((Boolean) y).booleanValue(); }
			if (y instanceof ScriptableObject)
			{
				Object test = ((ScriptableObject) y).equivalentValues(x);
				if (test != Scriptable.NOT_FOUND) { return ((Boolean) test).booleanValue(); }
		...
		else if (x instanceof Scriptable)
		{
			if (y instanceof Scriptable)
			{
				...
				// UNWRAP AND CALL EQUALS not only ==
				if (x instanceof Wrapper && y instanceof Wrapper)
				{
					Object o1 = ((Wrapper) x).unwrap();
					Object o2 = ((Wrapper) y).unwrap();
			...
			else if (y instanceof String) { return eqString((String) y, x); }
			...
			if (x instanceof Date && y instanceof Date) { return ((Date) x).getTime() == ((Date) y).getTime(); }
			if (unwrapped && (x != null && y != null)) { return x.equals(y); }
	...
	public static boolean shallowEq(Object x, Object y)
	{
		...
		{
			if (y instanceof Number) { return ((Number) x).doubleValue() == ((Number) y).doubleValue(); }
		}
		else if (x instanceof String)
		{
			if (y instanceof String) { return x.equals(y); }
		}
		else if (x instanceof Boolean)
		{
			if (y instanceof Boolean) { return x.equals(y); }
		}
		else if (x instanceof Scriptable)
		{
			if (x instanceof Wrapper && y instanceof Wrapper)
	...
	public static boolean instanceOf(Object a, Object b, Context cx)
	{
		...
		if (!(b instanceof Scriptable)) { throw typeError0("msg.instanceof.not.object"); }
	...
	/**
	 * The in operator. This is a new JS 1.3 language feature. The in operator
	 * mirrors the operation of the for .. in construct, and tests whether the
	 * rhs has the property given by the lhs. It is different from the for .. in
	 * construct in that: <BR> - it doesn't perform ToObject on the right hand
	 * side <BR> - it returns true for DontEnum properties.
	 * 
	 * @param a
	...
	public static boolean cmp_LT(Object val1, Object val2)
	{
		...
		else
		{
			...
			if (val1 instanceof String && val2 instanceof String) { return ((String) val1).compareTo((String) val2) < 0; }
			d1 = toNumber(val1);
			d2 = toNumber(val2);
		}
		return d1 < d2;
			...
			Scriptable scope)
	{
		...
		else
		{
			...
			if (javaException != null)
			{
				...
				ScriptableObject.defineProperty(errorObject, "javaException", wrap, ScriptableObject.PERMANENT
						| ScriptableObject.READONLY);
			}
			Object wrap = cx.getWrapFactory().wrap(cx, scope, re, null);
			ScriptableObject.defineProperty(errorObject, "rhinoException", wrap, ScriptableObject.PERMANENT
					| ScriptableObject.READONLY);
		...
		catchScopeObject.defineProperty("__exception__", Context.javaToJS(t, catchScopeObject),
	...
	{
		Scriptable sobj = toObjectOrNull(cx, obj);
		if (sobj == null) { throw typeError1("msg.undef.with", toString(obj)); }
	...
	{
		if (!(value instanceof XMLObject)) { throw notXmlError(value); }
	...
	public static Scriptable newArrayLiteral(Object[] objects, int[] skipIndexces, Context cx, Scriptable scope)
	{
		...
		Scriptable arrayObj;
		/*
		 * If the version is 120, then new Array(4) means create a new array with
		 * 4 as the first element. In this case, we have to set length property
		 * manually.
		 */
	...
	{
		String idStr = (id == null) ? "null" : id.toString();
		String objectStr = (object == null) ? "null" : toString(object);
		return typeError2("msg.undef.method.call", objectStr, idStr);
	}
	...
	public static RuntimeException notFunctionError(Object value, Object messageHelper)
	{
		...
		if (value == Scriptable.NOT_FOUND) { return typeError1("msg.function.not.found", msg); }
	...
	public static RegExpProxy checkRegExpProxy(Context cx)
	{
		...
		if (result == null) { throw Context.reportRuntimeError0("msg.no.regexp"); }
		return result;
	}
	...
	{
		if (!(obj instanceof XMLObject)) { throw notXmlError(obj); }
		XMLObject xmlObject = (XMLObject) obj;
