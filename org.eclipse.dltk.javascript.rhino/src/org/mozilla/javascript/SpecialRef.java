...
class SpecialRef extends Ref
{
	...
	static Ref createSpecial(Context cx, Object object, String name)
	{
		...
		if (target == null) { throw ScriptRuntime.undefReadError(object, name); }
		...
		switch (type)
		{
			...
			case SPECIAL_PARENT:
			{
				...
				if (obj != null)
				{
					...
					do
					{
						if (search == target) { throw Context.reportRuntimeError1("msg.cyclic.value", name); }
	...
	{
		if (type == SPECIAL_NONE) { return ScriptRuntime.hasObjectElem(target, name, cx); }
		return true;
	}

	public boolean delete(Context cx)
	{
		if (type == SPECIAL_NONE) { return ScriptRuntime.deleteObjectElem(target, name, cx); }
		return false;
	}
}
