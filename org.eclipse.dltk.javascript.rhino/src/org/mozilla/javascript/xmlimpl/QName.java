...
final class QName extends IdScriptableObject {
	...
	static QName create(XMLLibImpl lib, Scriptable scope, QName prototype,
			XmlNode.QName delegate) {
		QName rv = new QName();
		rv.lib = lib;
	...
	QName newQName(XMLLibImpl lib, String q_uri, String q_localName,
			String q_prefix) {
		...
		if (q_localName != null && q_localName.equals("*"))
			q_localName = null;
		return create(lib, this.getParentScope(), prototype,
				XmlNode.QName.create(ns, q_localName));
	}

	// See ECMA357 13.3.2
	QName constructQName(XMLLibImpl lib, Context cx, Object namespace,
			Object name) {
		...
		if (namespace == null) {
			...
			namespaceNamespace = lib.newNamespace(ScriptRuntime
					.toString(namespace));
		}
