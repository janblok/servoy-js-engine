...
class Namespace extends IdScriptableObject {
	...
	static Namespace create(Scriptable scope, Namespace prototype,
	...
	Namespace newNamespace(String uri) {
		...
		return create(this.getParentScope(), prototype,
				XmlNode.Namespace.create(uri));
	}
	...
	Namespace newNamespace(String prefix, String uri) {
		...
		Namespace prototype = (this.prototype == null) ? this : this.prototype;
		return create(this.getParentScope(), prototype,
				XmlNode.Namespace.create(prefix, uri));
	}
	...
	Namespace constructNamespace(Object uriValue) {
		...
		if (uriValue instanceof Namespace) {
			...
			uri = qname.uri();
			if (uri != null) {
				// TODO Is there a way to push this back into QName so that we
				// can make prefix() private?
				prefix = qname.prefix();
			} else {
				uri = qname.toString();
				prefix = null;
