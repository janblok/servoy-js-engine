...
class XMLName extends Ref {
	...
	static XMLName create(XmlNode.QName qname, boolean attribute,
			boolean descendants) {
		XMLName rv = new XMLName();
		rv.qname = qname;
					...
					for (int i = 0; i < xmlValue.length(); i++) {
						...
						if (xml.isAttribute()) {
							((XMLList) xmlValue).replace(
									i,
									target.makeXmlFromString(xmlName,
											xml.toString()));
						}
					}
				}
	...
	final boolean matches(XML node) {
		...
		if (isAttributeName) {
			if (node.isAttribute()) {
				...
				// TODO Could throw exception maybe, should not call this method
				// on attribute name with arbitrary node type
				// unless we traverse all attributes and children habitually
				return false;
			}
		} else {
			if (this.uri() == null
					|| ((node.isElement()) && this.uri().equals(nodeUri))) {
				if (localName().equals("*"))
					return true;
				if (node.isElement()) {
