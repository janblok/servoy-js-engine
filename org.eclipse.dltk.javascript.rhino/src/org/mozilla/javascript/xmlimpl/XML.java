...
class XML extends XMLObjectImpl {
	...
	boolean hasXMLProperty(XMLName xmlName) {
		if (isPrototype()) {
			...
			return (getPropertyList(xmlName).length() > 0)
					|| (getMethod(xmlName.localName()) != NOT_FOUND);
		}
	}
	...
	protected Object jsConstructor(Context cx, boolean inNewExpr, Object[] args) {
		...
		// ECMA 13.4.2 does not appear to specify what to do if multiple
		// arguments are sent.
	...
	void setAttribute(XMLName xmlName, Object value) {
		...
		this.node
				.setAttribute(xmlName.toQname(), ScriptRuntime.toString(value));
	}
	...
	XMLList elements(XMLName name) {
		...
		XmlNode[] elements = this.node
				.getMatchingChildren(XmlNode.Filter.ELEMENT);
	...
	XMLList child(XMLName xmlName) {
		...
		// TODO Should this also match processing instructions? If so, we have
		// to change the filter and also the XMLName
	...
	boolean equivalentXml(Object target) {
		...
		if (target instanceof XML) {
			// TODO This is a horrifyingly inefficient way to do this so we
			// should make it better. It may also not work.
			return this.node.toXmlString(getProcessor()).equals(
					((XML) target).node.toXmlString(getProcessor()));
	...
	XMLList processingInstructions(XMLName xmlName) {
		...
		this.node.addMatchingChildren(rv,
				XmlNode.Filter.PROCESSING_INSTRUCTION(xmlName));
		return rv;
	}
	...
	private XmlNode[] getNodesForInsert(Object value) {
		if (value instanceof XML) {
			...
			return new XmlNode[] { XmlNode.createText(getProcessor(),
	...
	XML setChildren(Object xml) {
		// TODO Have not carefully considered the spec but it seems to call for
		// this
		if (!isElement())
			return this;
	...
	void setName(QName name) {
		...
		if (isText() || isComment())
			return;
		if (isProcessingInstruction()) {
			// Spec says set the name URI to empty string and then set the
			// [[Name]] property, but I understand this to do the same
			// thing, unless we allow colons in processing instruction targets,
			// which I think we do not.
