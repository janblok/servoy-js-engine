...
abstract class XMLObjectImpl extends XMLObject {
	...
	Id_addNamespace = 2, Id_appendChild = 3, Id_attribute = 4,
			...
			Id_descendants = 12, Id_elements = 13, Id_inScopeNamespaces = 14,
			Id_insertChildAfter = 15, Id_insertChildBefore = 16,
			Id_hasOwnProperty = 17, Id_hasComplexContent = 18,
			Id_hasSimpleContent = 19, Id_length = 20, Id_localName = 21,
			Id_name = 22, Id_namespace = 23, Id_namespaceDeclarations = 24,
			Id_nodeKind = 25, Id_normalize = 26, Id_parent = 27,
			Id_prependChild = 28, Id_processingInstructions = 29,
	...
	public Object execIdCall(IdFunctionObject f, Context cx, Scriptable scope,
		...
		case Id_attribute: {
			XMLName xmlName = XMLName.create(
					lib.toNodeQName(cx, arg(args, 0), true), true, false);
			return realThis.getMatches(xmlName);
		}
		case Id_attributes:
			return realThis.getMatches(XMLName.create(
					XmlNode.QName.create(null, null), true, false));
		case Id_child: {
			XMLName xmlName = lib.toXMLNameOrIndex(cx, arg(args, 0));
			if (xmlName == null) {
		...
		case Id_descendants: {
			XmlNode.QName qname = (args.length == 0) ? XmlNode.QName.create(
					null, null) : lib.toNodeQName(cx, args[0], false);
	...
	final XML newTextElementXML(XmlNode reference, XmlNode.QName qname,
			String value) {
