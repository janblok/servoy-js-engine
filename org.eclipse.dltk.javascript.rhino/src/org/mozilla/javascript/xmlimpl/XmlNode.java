...
class XmlNode {
	...
	static XmlNode newElementWithText(XmlProcessor processor,
			XmlNode reference, XmlNode.QName qname, String value) {
		if (reference instanceof org.w3c.dom.Document)
			throw new IllegalArgumentException(
					"Cannot use Document node as reference");
	...
	static XmlNode createElement(XmlProcessor processor, String namespaceUri,
			String xml) throws org.xml.sax.SAXException {
	...
	int getChildIndex() {
		...
		if (parent() == null)
			return -1;
		org.w3c.dom.NodeList siblings = this.dom.getParentNode()
				.getChildNodes();
		...
		// Either the parent is -1 or one of the this node's parent's children
		// is this node.
	...
	void deleteMe() {
		if (dom instanceof Attr) {
			...
			attr.getOwnerElement()
					.getAttributes()
					.removeNamedItemNS(attr.getNamespaceURI(),
							attr.getLocalName());
		} else {
			if (this.dom.getParentNode() != null) {
				this.dom.getParentNode().removeChild(this.dom);
			} else {
				// This case can be exercised at least when executing the
				// regression
				// tests under
	...
	boolean isSameNode(XmlNode other) {
		// TODO May need to be changed if we allow XmlNode to refer to several
		// Node objects
	...
	private void addNamespaces(Namespaces rv, Element element) {
		...
		if (element.getParentNode() != null) {
			parentDefaultNamespace = toUri(element.getParentNode()
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (attr.getPrefix() != null && attr.getPrefix().equals("xmlns")) {
				rv.declare(Namespace.create(attr.getLocalName(),
						attr.getValue()));
			}
		}
	}
	...
	Namespace getNamespaceDeclaration(String prefix) {
		if (prefix.equals("") && dom instanceof Attr) {
			// Default namespaces do not apply to attributes; see XML Namespaces
			// section 5.2
	...
	private static class Namespaces {
		...
		void declare(Namespace n) {
			...
			// TODO I think this is analogous to the other way, but have not
			// really thought it through ... should local scope
	...
	final boolean isTextType() {
		return dom.getNodeType() == Node.TEXT_NODE
				|| dom.getNodeType() == Node.CDATA_SECTION_NODE;
	}
	...
	void invalidateNamespacePrefix() {
		...
		QName after = QName.create(this.dom.getNamespaceURI(),
				this.dom.getLocalName(), null);
		renameNode(after);
		NamedNodeMap attrs = this.dom.getAttributes();
		for (int i = 0; i < attrs.getLength(); i++) {
			if (attrs.item(i).getPrefix().equals(prefix)) {
				createImpl(attrs.item(i)).renameNode(
						QName.create(attrs.item(i).getNamespaceURI(), attrs
								.item(i).getLocalName(), null));
			}
		}
	}
	...
	private void declareNamespace(Element e, String prefix, String uri) {
		if (prefix.length() > 0) {
			e.setAttributeNS(XML_NAMESPACES_NAMESPACE_URI, "xmlns:" + prefix,
					uri);
		} else {
	...
	void declareNamespace(String prefix, String uri) {
		if (!(dom instanceof Element))
			throw new IllegalStateException();
		if (dom.lookupNamespaceURI(uri) != null
	...
	private Namespace getDefaultNamespace() {
		String prefix = "";
		String uri = (dom.lookupNamespaceURI(null) == null) ? "" : dom
				.lookupNamespaceURI(null);
	...
	void removeNamespace(Namespace namespace) {
		...
		// TODO I must confess I am not sure I understand the spec fully. See
		// ECMA357 13.4.4.31
		String existingPrefix = getExistingPrefixFor(namespace);
		if (existingPrefix != null) {
			if (namespace.isUnspecifiedPrefix()) {
				// we should remove any namespace with this URI from scope; we
				// do this by declaring a namespace with the same
				// prefix as the existing prefix and setting its URI to the
				// default namespace
				declareNamespace(existingPrefix, getDefaultNamespace().getUri());
			} else {
				if (existingPrefix.equals(namespace.getPrefix())) {
					declareNamespace(existingPrefix, getDefaultNamespace()
							.getUri());
				}
			}
		} else {
			// the argument namespace is not declared in this scope, so do
			// nothing.
		}
	}
	...
	private void setProcessingInstructionName(String localName) {
		...
		// We cannot set the node name; Document.renameNode() only supports
		// elements and attributes. So we replace it
		pi.getParentNode().replaceChild(
				pi,
				pi.getOwnerDocument().createProcessingInstruction(localName,
						pi.getData()));
	}
	...
	final void setLocalName(String localName) {
		if (dom instanceof ProcessingInstruction) {
			...
			this.dom = dom.getOwnerDocument().renameNode(dom,
					dom.getNamespaceURI(), QName.qualify(prefix, localName));
		}
	}

	final QName getQname() {
		String uri = (dom.getNamespaceURI()) == null ? "" : dom
				.getNamespaceURI();
	...
	void replaceWith(XmlNode other) {
		...
		if (replacement.getOwnerDocument() != this.dom.getOwnerDocument()) {
			replacement = this.dom.getOwnerDocument().importNode(replacement,
					true);
		}
	...
	String ecmaToXMLString(XmlProcessor processor) {
		if (this.isElementType()) {
			...
			for (int i = 0; i < inScope.length; i++) {
				declareNamespace(copy, inScope[i].getPrefix(),
						inScope[i].getUri());
			}
	...
	static class Namespace {
		...
		boolean is(Namespace other) {
			return this.prefix != null && other.prefix != null
	...
	static class QName {
		...
		void lookupPrefix(org.w3c.dom.Node node) {
			...
			while (prefix == null) {
				...
				if (generatedUri == null) {
					...
					((org.w3c.dom.Element) top).setAttributeNS(
			...
			if (namespace.getPrefix() == null) {
				if (node != null) {
					...
					if (namespace.getUri().equals("")) {
						...
						// TODO I am not sure this is right, but if we are
						// creating a standalone node, I think we can set the
						// default namespace on the node itself and not worry
						// about setting a prefix for that namespace.
						namespace.setPrefix("");
					}
				}
			}
		...
		void setAttribute(org.w3c.dom.Element element, String value) {
			...
			element.setAttributeNS(namespace.getUri(),
