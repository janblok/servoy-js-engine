...
public final class NativeDate extends IdScriptableObject implements Wrapper
{
	...
	public Object execIdCall(IdFunctionObject f, Context cx, Scriptable scope, Scriptable thisObj, Object[] args)
	{
		...
		switch (id)
		{
			...
			case Id_toTimeString:
			case Id_toDateString:
				if (t == t) { return date_format(t, id); }
				return js_NaN_date_str;

			case Id_toLocaleString:
			case Id_toLocaleTimeString:
			case Id_toLocaleDateString:
				if (t == t) { return toLocale_helper(t, id); }
				return js_NaN_date_str;
	...
	{
		return ((365 * ((y) - 1970) + Math.floor(((y) - 1969) / 4.0) - Math.floor(((y) - 1901) / 100.0) + Math
				.floor(((y) - 1601) / 400.0)));
	}
	...
	private static int YearFromTime(double t)
	{
		...
		/*
		 * Use a simple binary search algorithm to find the right year. This seems
		 * like brute force... but the computation of hi and lo years above lands
		 * within one year of the correct answer for years within a thousand years
		 * of 1970; the loop below only requires six iterations for year 270000.
		 */
		while (hi > lo)
		{
			mid = (hi + lo) / 2;
			...
			{
				lo = mid + 1;
				if (TimeFromYear(lo) > t) { return mid; }
			}
		}
		return lo;
	}
	...
	private static int DateFromTime(double t)
	{
		...
		d -= 31 + 28;
		if (d < 0) { return (d < -28) ? d + 31 + 28 + 1 : d + 28 + 1; }
	 ...
	 * clean way to make toLocaleString() and the time zone (comment) in toString
	 * match the generated string values. Currently it's wrong-but-consistent in
	 * all but the most recent betas of the JRE - seems to work in 1.1.7.
	 */
	...
	private static double DaylightSavingTA(double t)
	{
		...
		else
		{
			 ...
			 * requires (expensive) exploded date arguments, and the api makes it
			 * impossible to handle dst changeovers cleanly.
			 */
			...
			int year = YearFromTime(t);
			double offset = thisTimeZone.getOffset(year > 0 ? 1 : 0, year, MonthFromTime(t), DateFromTime(t), WeekDay(t),
					(int) TimeWithinDay(t));
	...
	{
		if (d != d || d == Double.POSITIVE_INFINITY || d == Double.NEGATIVE_INFINITY || Math.abs(d) > HalfTimeDomain) { return ScriptRuntime.NaN; }
		if (d > 0.0)
	...
	private static double date_msecFromArgs(Object[] args)
	{
		...
		for (loop = 0; loop < MAXARGS; loop++)
		{
			if (loop < args.length)
			{
				...
				if (d != d || Double.isInfinite(d)) { return ScriptRuntime.NaN; }
	...
	private static double date_parseString(String s)
	{
		...
		while (i < limit)
		{
			...
			else
			{
				 ...
				 * date-parsing code from Java, to keep js and rhino consistent. Is
				...
				{
					/*
					 * AM/PM. Count 12:30 AM as 00:30, 12:30 PM as 12:30, instead of
					 * blindly adding 12 if PM.
					 */
					if (hour > 12 || hour < 0)
					{
						return ScriptRuntime.NaN;
	...
	private static double makeTime(double date, Object[] args, int methodId)
	{
		...
		/*
		 * Satisfy the ECMA rule that if a function is called with fewer arguments
		 * than the specified formal arguments, the remaining arguments are set to
		 * undefined. Seems like all the Date.setWhatever functions in ECMA are
		 * only varargs beyond the first argument; this should be set to undefined
		...
		for (i = 0; i < args.length && i < maxargs; i++)
		{
			...
			if (conv[i] != conv[i] || Double.isInfinite(conv[i])) { return ScriptRuntime.NaN; }
	...
	Id_constructor = 1, Id_toString = 2, Id_toTimeString = 3, Id_toDateString = 4, Id_toLocaleString = 5,
			...
			Id_getUTCMonth = 16, Id_getDate = 17, Id_getUTCDate = 18, Id_getDay = 19, Id_getUTCDay = 20, Id_getHours = 21,
			Id_getUTCHours = 22, Id_getMinutes = 23, Id_getUTCMinutes = 24, Id_getSeconds = 25, Id_getUTCSeconds = 26,
			Id_getMilliseconds = 27, Id_getUTCMilliseconds = 28, Id_getTimezoneOffset = 29, Id_setTime = 30,
			Id_setMilliseconds = 31, Id_setUTCMilliseconds = 32, Id_setSeconds = 33, Id_setUTCSeconds = 34,
			Id_setMinutes = 35, Id_setUTCMinutes = 36, Id_setHours = 37, Id_setUTCHours = 38, Id_setDate = 39,
			Id_setUTCDate = 40, Id_setMonth = 41, Id_setUTCMonth = 42, Id_setFullYear = 43, Id_setUTCFullYear = 44,
			Id_setYear = 45,
