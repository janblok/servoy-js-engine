		...
		try
		{
			...
			if (context == LOCAL_CONTEXT_ID || context == -1)
			{
				...
				for (int a = 0; a < propertyIds.length; a++) {
					...
					if (!(value instanceof Function) || (value instanceof XMLObject)) // HACK because ShowFunctionsAction doesnt work because of the lazy behavior of plugins in Eclipse
					{
		...
		if(scriptable != null)
		{
			...
			for (int i = 0; i < ids.length; i++)
			{
				...
				if (!doubles.contains(id))
				{
					...
					{
						value = scriptable.get(((Number)id).intValue(),scriptable);
					}
					else
					{
			...
			sendAllIds(properties, stackFrame, scriptable.getPrototype(),doubles,walkParents);
			if (walkParents) sendAllIds(properties, stackFrame, scriptable.getParentScope(),doubles,walkParents);
		}
	}
}