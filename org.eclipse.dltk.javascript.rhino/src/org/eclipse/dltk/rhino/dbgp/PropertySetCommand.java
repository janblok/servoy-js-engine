...
final class PropertySetCommand extends DBGPDebugger.Command {
	...
	void parseAndExecute(String command, HashMap options) {
		...
		if (num >= 0) {
			DBGPDebugFrame fr = this.debugger.getStackManager().getStackFrame(num);
			fr.setValue(name, value);
		} else {
			if (name.equals("suspendOnException")) {
				boolean parseBoolean = new Boolean(value).booleanValue();
				this.debugger.getStackManager().getManager().setSuspendOnException(parseBoolean);
			}
			else if (name.equals("suspendOnEntry")) {
				boolean parseBoolean = new Boolean(value).booleanValue();
				this.debugger.getStackManager().getManager().setSuspendOnEntry(parseBoolean);
			}
			else if (name.equals("suspendOnExit")) {
				boolean parseBoolean = new Boolean(value).booleanValue();
				this.debugger.getStackManager().getManager().setSuspendOnExit(parseBoolean);
			}
			else debugger.setProperty(name, value);
		}
